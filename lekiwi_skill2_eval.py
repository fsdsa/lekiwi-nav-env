...
class Skill2EnvCfg(DirectRLEnvCfg):
    ...
    grasp_ee_max_dist: float = 0.10  # EE(캘리브레이션 좌표)~물체 중심 거리 < 이 값이면 grasp 가능 (물체 내부 판정)
    grasp_bbox_margin: float = 0.01  # EE must be inside object bbox (+margin) to count as grasp
    grasp_require_object_standing: bool = True
    grasp_min_object_height: float = 0.026
    grasp_min_object_up_z: float = 0.75
    grasp_success_height: float = 0.05       # task_success 판정 높이 (순수 마찰 lift)
    lift_hold_steps: int = 0                 # lift 유지 step 수 (0=즉시 성공, >0이면 해당 step 연속 유지 시 성공)
    ...
    dest_heading_noise_std: float = 0.3
    dest_heading_max_rad: float = 0.5

    # === Scene (ProcTHOR 등 외부 USD 배경) ===
    use_builtin_ground: bool = True
    builtin_ground_z: float = 0.0
    scene_reference_usd: str = ""
    scene_prim_path: str = "/World/ProcTHORScene"
...
class Skill2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Skill2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.wrist_object_contact_sensor: ContactSensor | None = getattr(self, "wrist_object_contact_sensor", None)
        ...
        self.grasp_drop_counter = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self.lift_hold_counter = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def _setup_scene(self):
        ...
        self.wrist_object_contact_sensor = None
        ...
        if self._multi_object:
            ...
            wrist_contact_cfg = ContactSensorCfg(
                prim_path=wrist_prim,
                update_period=0.0,
                history_length=2,
                filter_prim_paths_expr=filter_exprs,
            )
            self.wrist_object_contact_sensor = ContactSensor(wrist_contact_cfg)
        ...
        gz = float(self.cfg.builtin_ground_z)
        if self.cfg.use_builtin_ground:
            ...
            ground_cfg.func("/World/ground", ground_cfg, translation=(0.0, 0.0, gz - 0.05))

        light_cfg = sim_utils.DomeLightCfg(intensity=1500.0, color=(0.9, 0.9, 0.9))
        light_cfg.func("/World/Light", light_cfg)

        # Scene reference (ProcTHOR 등) — clone 전에 로드해야 filter_collisions에 포함됨
        if self.cfg.scene_reference_usd and os.path.isfile(os.path.expanduser(self.cfg.scene_reference_usd)):
            import omni.usd
            from pxr import UsdPhysics
            stage = omni.usd.get_context().get_stage()
            scene_prim_path = str(self.cfg.scene_prim_path)
            if stage.GetPrimAtPath(scene_prim_path).IsValid():
                stage.RemovePrim(scene_prim_path)
            scene_prim = stage.DefinePrim(scene_prim_path, "Xform")
            scene_prim.GetReferences().AddReference(
                str(os.path.expanduser(self.cfg.scene_reference_usd)))
            stage.Load()
            # Scene scale 적용
            ss = float(self.cfg.scene_scale)
            if abs(ss - 1.0) > 1e-6:
                from pxr import UsdGeom, Gf
                xf = UsdGeom.Xformable(scene_prim)
                xf.ClearXformOpOrder()
                xf.AddScaleOp().Set(Gf.Vec3f(ss, ss, ss))
            # Scene 내 모든 rigid body → kinematic 변환 (물리 시뮬 방지)
            n_fixed = 0
            groups_to_remove = []
            for prim in stage.Traverse():
                if prim.GetPath().HasPrefix(scene_prim_path):
                    rb = UsdPhysics.RigidBodyAPI(prim) if prim.HasAPI(UsdPhysics.RigidBodyAPI) else None
                    if rb:
                        rb.GetKinematicEnabledAttr().Set(True)
                        n_fixed += 1
                    if prim.IsA(UsdPhysics.CollisionGroup):
                        groups_to_remove.append(str(prim.GetPath()))
            for path in groups_to_remove:
                stage.RemovePrim(path)
            print(f"  [Scene] Loaded: {self.cfg.scene_reference_usd}")
            print(f"  [Scene] {n_fixed} kinematic, {len(groups_to_remove)} collision groups removed")

        # Subclass hook: 추가 씬 오브젝트 생성 (clone 전)
        self._create_extra_scene_objects()

        self.scene.clone_environments(copy_from_source=False)
        if not self.cfg.scene_reference_usd:
            global_prims = []
            if self.cfg.use_builtin_ground:
                global_prims.append("/World/ground")
        ...
        if self.wrist_object_contact_sensor is not None:
            self.scene.sensors["wrist_object_contact"] = self.wrist_object_contact_sensor
        ...
        if getattr(self, "dest_contact_sensor", None) is not None:
            self.scene.sensors["dest_contact"] = self.dest_contact_sensor
    ...
    def _create_extra_scene_objects(self):
        ...
        # Dest contact sensor: DestObject ↔ (gripper jaw + wrist roll + source object)
        self.dest_contact_sensor: ContactSensor | None = None
        dest_filter_exprs = []
        if str(self.cfg.gripper_contact_prim_path).strip():
            dest_filter_exprs.append(str(self.cfg.gripper_contact_prim_path))
            wrist_prim = str(self.cfg.gripper_contact_prim_path).replace(
                "Moving_Jaw_08d_v1", "Wrist_Roll_08c_v1")
            dest_filter_exprs.append(wrist_prim)
        if str(self.cfg.object_filter_prim_expr).strip():
            dest_filter_exprs.append(str(self.cfg.object_filter_prim_expr))
        if dest_filter_exprs:
            dest_contact_cfg = ContactSensorCfg(
                prim_path="/World/envs/env_.*/DestObject",
                update_period=0.0,
                history_length=2,
                filter_prim_paths_expr=dest_filter_exprs,
            )
            self.dest_contact_sensor = ContactSensor(dest_contact_cfg)
            print(f"  [Skill2Env] Dest contact sensor: filters={dest_filter_exprs}")
    ...
    def _sensor_force_per_env(self, sensor: ContactSensor | None) -> torch.Tensor:
        force = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
        if not self._physics_grasp or sensor is None:
            return force
    ...
    def _contact_force_per_env(self) -> torch.Tensor:
        return self._sensor_force_per_env(self.contact_sensor)

    def _wrist_contact_force_per_env(self) -> torch.Tensor:
        return self._sensor_force_per_env(self.wrist_object_contact_sensor)

    def _dest_contact_force_per_env(self) -> torch.Tensor:
        """DestObject ↔ (jaw + wrist + source object) contact force per env."""
        sensor = getattr(self, "dest_contact_sensor", None)
        if sensor is None:
            return torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
        return self._sensor_force_per_env(sensor)
    ...
    def _compute_metrics(self) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "object_quat_w": self.object_quat_w,
    ...
    def _update_grasp_state(self, metrics: Dict[str, torch.Tensor]):
        ...
        if not self._physics_grasp or self.contact_sensor is None:
            ...
            jaw_contact_force = self._contact_force_per_env()
            wrist_contact_force = self._wrist_contact_force_per_env()
            has_jaw_contact = jaw_contact_force > float(self.cfg.grasp_contact_threshold)
            has_wrist_contact = wrist_contact_force > float(self.cfg.grasp_contact_threshold)
            has_contact = has_jaw_contact & has_wrist_contact
            if bool(self.cfg.grasp_require_object_standing):
                object_standing = (
                    (metrics["object_height"] > float(self.cfg.grasp_min_object_height))
                    & (metrics["object_up_z"] > float(self.cfg.grasp_min_object_up_z))
                )
            else:
                object_standing = torch.ones_like(has_contact)
            ...
            if self._fixed_jaw_body_idx >= 0:
                ...
                ee_pos_obj = quat_apply_inverse(metrics["object_quat_w"], ee_pos - self.object_pos_w)
                half_bbox = 0.5 * self.object_bbox + float(self.cfg.grasp_bbox_margin)
                inside_bbox = (ee_pos_obj.abs() <= half_bbox).all(dim=-1)
                between_jaws = inside_bbox & (ee_to_obj < float(self.cfg.grasp_ee_max_dist))
            else:
                between_jaws = close_enough  # fallback: body 못 찾으면 기존 로직

            if self.cfg.grasp_require_contact:
                can_grasp = gripper_closed & has_contact & object_standing & between_jaws & (~self.object_grasped)
            else:
                can_grasp = gripper_closed & object_standing & between_jaws & (~self.object_grasped)

        newly_grasped = can_grasp
        if newly_grasped.any():
            self.object_grasped[newly_grasped] = True
            self.just_grasped[newly_grasped] = True
            self.grasp_drop_counter[newly_grasped] = 0
        ...
        if self._fixed_jaw_body_idx >= 0:
            ...
            ee_to_obj = metrics["object_dist"]

        if self.object_grasped.any():
            dropped_now = self.object_grasped & (
                (ee_to_obj > float(self.cfg.grasp_drop_detect_dist))
                | (self.robot.data.joint_pos[:, self.gripper_idx] > float(self.cfg.grasp_gripper_threshold) + 0.05)
            )
            self.grasp_drop_counter[dropped_now] += 1
            self.grasp_drop_counter[~dropped_now] = 0
            newly_dropped = self.object_grasped & (
                self.grasp_drop_counter >= int(max(self.cfg.grasp_drop_steps, 1))
            )
            if newly_dropped.any():
                self.object_grasped[newly_dropped] = False
                self.just_dropped[newly_dropped] = True
                self.grasp_drop_counter[newly_dropped] = 0
        ...
        if self.object_grasped.any():
            ...
            if self.cfg.lift_hold_steps > 0:
                # 연속 유지 모드: lifted 상태에서 카운터 증가, 아니면 리셋
                self.lift_hold_counter[lifted] += 1
                self.lift_hold_counter[~lifted] = 0
                self.task_success[self.lift_hold_counter >= self.cfg.lift_hold_steps] = True
            else:
    ...
    def _get_observations(self) -> dict:
        ...
        jaw_contact_force = self._contact_force_per_env()
        wrist_contact_force = self._wrist_contact_force_per_env()
        jaw_contact_binary = (jaw_contact_force > float(self.cfg.grasp_contact_threshold)).float()
        wrist_contact_binary = (wrist_contact_force > float(self.cfg.grasp_contact_threshold)).float()
        contact_lr = torch.stack([jaw_contact_binary, wrist_contact_binary], dim=-1)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self.extras["object_up_z_raw"] = metrics["object_up_z"].clone()
        self.extras["object_standing_mask"] = (
            (metrics["object_height"] > float(self.cfg.grasp_min_object_height))
            & (metrics["object_up_z"] > float(self.cfg.grasp_min_object_up_z))
        ).clone()
        # Contact sensor: lift sustain 판정용 (teleop 0.65와 동일 threshold)
        if self.cfg.grasp_require_contact:
            _jaw_cf = self._contact_force_per_env()
            _wrist_cf = self._wrist_contact_force_per_env()
            _dual_contact = (
                (_jaw_cf > float(self.cfg.lift_contact_threshold))
                & (_wrist_cf > float(self.cfg.lift_contact_threshold))
            )
            self.extras["has_contact_mask"] = _dual_contact.clone()
            self.extras["has_dual_contact_mask"] = _dual_contact.clone()
            self.extras["contact_force_raw"] = _jaw_cf.clone()
            self.extras["jaw_contact_force_raw"] = _jaw_cf.clone()
            self.extras["wrist_contact_force_raw"] = _wrist_cf.clone()
            self.extras["ee_inside_bbox_mask"] = torch.zeros(
            ...
            if self._fixed_jaw_body_idx >= 0:
                ...
                wrist_quat = self.robot.data.body_quat_w[:, self._fixed_jaw_body_idx, :]
                ee_pos = wrist_pos + quat_apply(wrist_quat, self._ee_local_offset.expand_as(wrist_pos))
                ee_pos_obj = quat_apply_inverse(self.object_quat_w, ee_pos - self.object_pos_w)
                half_bbox = 0.5 * self.object_bbox + float(self.cfg.grasp_bbox_margin)
                self.extras["ee_inside_bbox_mask"] = (ee_pos_obj.abs() <= half_bbox).all(dim=-1).clone()
            ...
            self.extras["has_dual_contact_mask"] = self.object_grasped.clone()
            self.extras["contact_force_raw"] = torch.zeros(self.num_envs, device=self.device)
            self.extras["jaw_contact_force_raw"] = torch.zeros(self.num_envs, device=self.device)
            self.extras["wrist_contact_force_raw"] = torch.zeros(self.num_envs, device=self.device)
            self.extras["ee_inside_bbox_mask"] = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            self.extras["ground_contact_force_raw"] = self._ground_contact_force_per_env().clone()
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        self.lift_hold_counter[env_ids] = 0
        # 실제 거리로 초기화 (10.0 하드코딩 → 공짜 첫 스텝 보상 방지)
        ...
        self.grasp_drop_counter[env_ids] = 0
