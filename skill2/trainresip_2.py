...
Based on Run 1 (v6.4) with targeted fixes:
  1. l_sus grace period — prevents flickering contact from killing lift rewards
  2. Better LIFTED_POSE target (actual carry pose from teleop)
  3. R3b Verified lift milestone (+100, one-time after sustained hold)
  4. R4b two-stage lifted-pose shaping (pre/post verified lift)
  5. R4c Pose return milestone (+300, one-time after pose hold)
  6. Lift height reward with min/max range (4-12cm)

Core v6 rewards UNCHANGED:
  R1  Gripper open milestone     +10    one-time, gates R2/R3
  R2  Arm approach (XY)          ×30    delta, base-subtracted
  R3  Verified grasp             +25    one-time, 5-step sustained (env eg flag)
  R4  Lift height                ×200   per-step, sustain≥3, grip closed, ee<0.13
...
parser.add_argument("--verified_lift_steps", type=int, default=50)
parser.add_argument("--gripper_open_threshold", type=float, default=0.8)
parser.add_argument("--held_ee_max_dist", type=float, default=0.13)
...
parser.add_argument("--l_sus_decay", type=int, default=3,
                    help="l_sus decay per step when held=False (grace period). 0=instant reset (old behavior)")
...
def main():
    ...
    L_SUS_DECAY = max(int(args.l_sus_decay), 0)

    # Lift reward target: actual carry pose from teleop
    LIFTED_POSE = torch.tensor([-0.045, -0.194, 0.277, -0.908, 0.020], device=dev)
    LIFT_REWARD_MIN_H = 0.04
    LIFT_REWARD_MAX_H = 0.12   # [FIX1] was 0.30 — cap height reward to prevent arm-stretching
    LIFT_POSE_SIGMA = 1.0     # [FIX2] 0.35→0.70 — balance between gradient strength and reachability
    ...
    ms_sl_t1 = torch.zeros(N, dtype=torch.bool, device=dev)
    ms_sl_t2 = torch.zeros(N, dtype=torch.bool, device=dev)
    ms_sl_t3 = torch.zeros(N, dtype=torch.bool, device=dev)
    ms_sl_t4 = torch.zeros(N, dtype=torch.bool, device=dev)
    ms_sl_t5 = torch.zeros(N, dtype=torch.bool, device=dev)
    ...
    def reset_ep(mask):
        ...
        ms_vl[mask] = False; ms_pr[mask] = False
        ms_li[mask] = False; ms_sl[mask] = False
        ms_sl_t1[mask] = False; ms_sl_t2[mask] = False; ms_sl_t3[mask] = False
        ms_sl_t4[mask] = False; ms_sl_t5[mask] = False
    ...
    print(f"  R1=GripOpen(+10,>{OPEN_T:.2f}) R2=ArmXY(×30,base-sub)")
    print(
        f"  R3=VGrasp(+{args.verified_grasp_bonus:.0f},{GV}s) "
        f"R3b=VLift(+{args.verified_lift_bonus:.0f},{VLS}s) "
    ...
    print(f"  l_sus_decay={L_SUS_DECAY} (0=instant reset)")
    ...
    while gs < args.total_timesteps:
        ...
        ms_go.zero_(); ms_gr.zero_(); ms_vl.zero_(); ms_pr.zero_()
        ms_li.zero_(); ms_sl.zero_()
        ms_sl_t1.zero_(); ms_sl_t2.zero_(); ms_sl_t3.zero_()
        ms_sl_t4.zero_(); ms_sl_t5.zero_()
        ...
        for step in range(S):
            ...
            # ══════════════════════════════════════════════════════
            # R1/R2/R3 REMOVED — v6.4b already knows grasp.
            # Grasp milestone tracking only (no reward)
            # ══════════════════════════════════════════════════════
            gc = eg & (~ms_gr)
            g_sus[gc] += 1
            g_sus[~gc & (~ms_gr)] = 0
            ...
            r_gr += vg.long()
            r_egr += (jg > 0).long()
            _open_ct += 0  # placeholder
            _r2_sum += 0.0

            # ══════════════════════════════════════════════════════
            # R4: LIFT HEIGHT (×200, 4~12cm, sustain≥3, ee<HELD_EE_MAX)
            # ══════════════════════════════════════════════════════
            ...
            l_sus[held] += 1
            if L_SUS_DECAY > 0:
                not_held_mask = ~held & (l_sus > 0)
                l_sus[not_held_mask] = torch.clamp(l_sus[not_held_mask] - L_SUS_DECAY, min=0)
            else:
                l_sus[~held] = 0
            ...
            # R3b: verified lift milestone
            vl = held & (l_sus >= VLS) & (~ms_vl)
            rew += vl.float() * args.verified_lift_bonus
            ms_vl |= vl
            r_vl += vl.long()

            # R4: lift height reward
            rew += ms_gr.float() * held.float() * lok.float() * hp * 200.0

            # ══════════════════════════════════════════════════════
            # R4b: TWO-STAGE LIFTED-POSE SHAPING [FIX3]
            #   Pre-verified-lift: 15% scale (focus on lifting first)
            #   Post-verified-lift: 100% scale (converge to carry pose)
            # ══════════════════════════════════════════════════════
            ...
            # ══════════════════════════════════════════════════════
            # R5: TIERED SUSTAINED LIFT MILESTONES
            # ══════════════════════════════════════════════════════
            t1 = held & (l_sus >= 15) & (~ms_sl_t1)
            rew += t1.float() * 10.0
            ms_sl_t1 |= t1

            t2 = held & (l_sus >= 50) & (~ms_sl_t2)
            rew += t2.float() * 30.0
            ms_sl_t2 |= t2

            t3 = held & (l_sus >= 100) & (~ms_sl_t3)
            rew += t3.float() * 60.0
            ms_sl_t3 |= t3

            t4 = held & (l_sus >= 200) & (~ms_sl_t4)
            rew += t4.float() * 150.0
            ms_sl_t4 |= t4

            t5 = held & (l_sus >= 500) & (~ms_sl_t5)
            rew += t5.float() * 500.0
            ms_sl_t5 |= t5
            ...
            # ══════════════════════════════════════════════════════
            # R8: GRIPPER-GROUND CONTACT PENALTY (always active)
            # ══════════════════════════════════════════════════════
            grip_on_ground = (gcf > 1.0)
