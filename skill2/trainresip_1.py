...
v6 original + 3 changes only:
  1. R4b Lifted pose (×30, Gaussian σ=2.0) — per-step during lift
  2. R8  Ground contact penalty (−0.5)   — blocks floor-press exploit
  3. Warmup: 3600 steps (2×1800), decay 60 iters — 2 full BC attempts per iter
...
def main():
    ...
    while gs < args.total_timesteps:
        ...
        # 3600 total = 2 full BC attempts (avoid wasting on 7500-step failed eps)
        WU_RESET = 1800
        for wi in range(ws):
            with torch.no_grad():
        ...
        for step in range(S):
            ...
            # ══════════════════════════════════════════════════════
            # R4b: LIFTED POSE APPROACH (×30, Gaussian σ=2.0) [v6.4 NEW]
            # ══════════════════════════════════════════════════════
