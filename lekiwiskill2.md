...
class Skill2EnvCfg(DirectRLEnvCfg):
    ...
    grasp_ee_max_dist: float = 0.10  # EE(캘리브레이션 좌표)~물체 중심 거리 < 이 값이면 grasp 가능
...
class Skill2Env(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # Ground contact sensor — gripper가 지면과 접촉하는지 감지
    ...
    def _ground_contact_force_per_env(self) -> torch.Tensor:
        """Gripper-ground contact force per env. 0 if sensor unavailable."""
        force = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
        if self.ground_contact_sensor is None:
            return force
        force_matrix = self.ground_contact_sensor.data.force_matrix_w
        if force_matrix is not None:
            mag = torch.norm(force_matrix, dim=-1)
            mag = mag.reshape(mag.shape[0], -1).sum(dim=-1)
            if mag.shape[0] == self.num_envs:
            ...
            if mag.numel() % self.num_envs == 0:
                return mag.reshape(self.num_envs, -1).sum(dim=-1)
        net = self.ground_contact_sensor.data.net_forces_w
        if net is not None:
