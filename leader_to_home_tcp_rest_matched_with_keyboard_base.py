...
# Isaac Sim joint positions at Play with no input.
SIM_REST_RAD6 = [
...
# Leader arm positions when matching Isaac Sim rest pose.
LEADER_REST_RAD6 = [
...
SPEED_STEP = 0.02  # m/s per key press
ANG_STEP = 0.5  # rad/s per key press
MIN_LINEAR = 0.02
MIN_ANG = 0.5
MAX_LINEAR = 0.5
MAX_ANG = 10.0
    ...
    if isinstance(raw_action, dict):
        ...
        def pick(name: str) -> float:
            for key, value in raw_action.items():
                ...
                if key_norm.startswith("arm_"):
                    key_norm = key_norm[4:]
                if key_norm.endswith(".pos"):
                    key_norm = key_norm[:-4]
                if key_norm == name:
                    try:
                        return float(value)
