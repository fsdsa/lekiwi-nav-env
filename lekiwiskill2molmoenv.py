...
class Skill2EnvCfg(DirectRLEnvCfg):
    ...
    use_builtin_ground: bool = True         # MolmoSpaces 최종 검증에서는 False로 꺼서 scene floor만 사용
    builtin_ground_z: float = 0.0          # ground cuboid 상단 면 Z 좌표 (scene floor_z에 맞춤)
    scene_reference_usd: str = ""           # ProcTHOR 등 외부 scene USD — 설정 시 _setup_scene에서 로드
    scene_prim_path: str = "/World/ProcTHORScene"  # scene reference가 로드될 prim path
    scene_scale: float = 1.0               # scene 전체 스케일 (0.7 = 70% 축소)
...
class Skill2Env(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        if self.cfg.use_builtin_ground and str(self.cfg.gripper_contact_prim_path).strip():
        ...
        # builtin_ground_z: ground 상단 면 Z (default=0.0, scene 사용 시 floor_z로 설정)
        ...
        if self.cfg.scene_reference_usd and os.path.isfile(os.path.expanduser(self.cfg.scene_reference_usd)):
            ...
            # ★ Payload 로드 (Physics.usda 등 서브레이어 활성화)
            stage.Load()
            # Scene scale 적용
            ...
            # Scene 내부 처리:
            #   1) rigid body → kinematic (가구 추락 방지)
            #   2) scene 자체 collision group 삭제 (AI2-THOR용이라 Isaac Lab과 비호환)
            # ★ 벽/바닥은 static collider 그대로 유지 — CPU PhysX에서 정상 충돌
            n_fixed = 0
            groups_to_remove = []
            for prim in stage.Traverse():
                if not str(prim.GetPath()).startswith(scene_prim_path):
                    continue
            ...
            print(f"  [Scene] Loaded: {self.cfg.scene_reference_usd} → {scene_prim_path}")
        ...
        # CPU + num_envs=1: filter_collisions 스킵 (inter-env 충돌 필터링 불필요)
        # GPU에서만 filter_collisions 사용
        if str(self.cfg.sim.device).startswith("cuda"):
            global_prims = []
            if self.cfg.use_builtin_ground:
                global_prims.append("/World/ground")
            if self.cfg.scene_reference_usd:
                global_prims.append(str(self.cfg.scene_prim_path))
            self.scene.filter_collisions(global_prim_paths=global_prims)
        else:
            print("  [Scene] CPU mode: skipping filter_collisions")
        ...
        # Subclass hook: 추가 rigid object + contact sensors 등록 (clone 후)
        ...
        if hasattr(self, "gripper_dest_contact_sensor") and self.gripper_dest_contact_sensor is not None:
            self.scene.sensors["gripper_dest_contact"] = self.gripper_dest_contact_sensor
        if hasattr(self, "wrist_dest_contact_sensor") and self.wrist_dest_contact_sensor is not None:
            self.scene.sensors["wrist_dest_contact"] = self.wrist_dest_contact_sensor
    ...
    def _create_extra_scene_objects(self):
        ...
        # Contact sensors: gripper ↔ dest object, wrist ↔ dest object
        dest_filter = "/World/envs/env_.*/DestObject"
        self.gripper_dest_contact_sensor: ContactSensor | None = None
        self.wrist_dest_contact_sensor: ContactSensor | None = None
        if str(self.cfg.gripper_contact_prim_path).strip():
            gripper_dest_cfg = ContactSensorCfg(
                prim_path=str(self.cfg.gripper_contact_prim_path),
                update_period=0.0,
                history_length=2,
                filter_prim_paths_expr=[dest_filter],
            )
            self.gripper_dest_contact_sensor = ContactSensor(gripper_dest_cfg)
            wrist_prim = str(self.cfg.gripper_contact_prim_path).replace(
                "Moving_Jaw_08d_v1", "Wrist_Roll_08c_v1")
            wrist_dest_cfg = ContactSensorCfg(
                prim_path=wrist_prim,
                update_period=0.0,
                history_length=2,
                filter_prim_paths_expr=[dest_filter],
            )
            self.wrist_dest_contact_sensor = ContactSensor(wrist_dest_cfg)
            print(f"  [Skill2Env] Dest contact sensors created (gripper + wrist)")
    ...
    def _dest_contact_force_per_env(self) -> torch.Tensor:
        """Gripper+Wrist contact force with dest object per env. 0 if sensors unavailable."""
        force = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device)
        for sensor in [
            getattr(self, "gripper_dest_contact_sensor", None),
            getattr(self, "wrist_dest_contact_sensor", None),
        ]:
            if sensor is None:
                continue
            fm = sensor.data.force_matrix_w
    ...
    def _spawn_dest_object(self, env_ids: torch.Tensor):
        """목적지 물체를 로봇 전방 heading 기준 랜덤으로 스폰 (source와 최소 거리 보장)."""
        ...
        # LeKiwi forward = +Y body → world yaw
    ...
    def _apply_action(self):
        ...
        # ── Temporary diagnostic: gripper target vs actual (first 10 calls) ──
        if not hasattr(self, "_diag_apply_ct"):
            self._diag_apply_ct = 0
        if self._diag_apply_ct < 10:
            grip_tgt = arm_targets[:, 5]  # gripper is index 5 in arm_idx
            grip_act = self.robot.data.joint_pos[:, self.gripper_idx].view(-1)
            grip_action_raw = arm_grip_action[:, 5]
            print(f"  [APPLY #{self._diag_apply_ct}] grip_action_raw={grip_action_raw[:4].tolist()} "
                  f"grip_target={grip_tgt[:4].tolist()} grip_actual={grip_act[:4].tolist()}")
            self._diag_apply_ct += 1
