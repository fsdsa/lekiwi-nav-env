...
**Navigate 보상 구조 (Isaac Lab style, v6g)**: v6f base(lin_std=0.25, turn_lin_drift=-3.0) + orthogonal drift penalty(`ortho_drift_weight=-1.0`). (1) linear tracking(1.5): `exp(-||target_lin - actual_lin||² / 0.25²)` — 선형 속도 추종, (2) angular tracking(1.5): `exp(-(target_wz - actual_wz)² / 0.20²)` — 각속도 추종, (3) orthogonal drift(-1.0): 목표 방향에 직교하는 속도 성분 억제 (`ortho_speed` TensorBoard 로그), (4) smoothness(-0.005): action delta² penalty, (5) time(-0.01): per-step cost. collision/proximity reward 제거 (장애물 회피는 VLA가 담당). 장애물 스폰은 유지(시각적 다양성). **direction_cmd turn magnitude**: ±0.33 (target wz = 0.33 × 3.0 = 1.0 rad/s). Early stop: `rew_track_ang >= 1.35 AND ang_vel_error <= 0.02`.

**Navigate v6g 비교 실험 (2026-02-24 진행중)**:
- **핵심 이슈**: 전후진(forward/backward) 시 뒷바퀴 drift 발생 — ortho penalty가 해결해야 할 문제. 좌우이동(strafe)과 회전은 잘 동작.
- **v6g2** (`ortho_drift_weight=-1.0`, linear penalty): 현재 코드 상태. v6g2_full 학습 서버에서 처음부터 완주 예정.
- **v6g3** (`ortho_drift_weight=-15.0`, quadratic penalty): 완주됨 → `backup/best_agent_v6g3.pt`
- **비교 목적**: ortho penalty 강도(1.0 vs 15.0)와 형태(linear vs quadratic)가 drift 억제와 tracking 성능에 미치는 영향 비교
- **기존 백업**: `best_agent_v6e.pt`, `best_agent_v6g2.pt` (17K steps 중간), `best_agent_v6g3.pt` (완주)
...
Phase 1: RL Expert 학습 (텔레옵: 3090 Desktop, BC/RL: A100 서버)
  ...
  [🔄] Skill-1 RL 학습 v6g 비교 실험 진행 중 (A100, 8192 envs)
        - v6g2 (ortho=-1.0, linear): 서버에서 v6g2_full 처음부터 완주 예정 ← 현재 코드 상태
        - v6g3 (ortho=-15.0, quadratic): 완주됨, backup/best_agent_v6g3.pt
        - 핵심 이슈: 전후진 drift → ortho penalty 비교 (좌우이동/회전은 정상)
        - 백업: best_agent_v6e.pt, best_agent_v6g2.pt(17K 중간), best_agent_v6g3.pt(완주)
