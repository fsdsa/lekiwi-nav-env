...
VLM이 이미지에서 물체의 시각적 크기로 대략 판단한다. ApproachAndGrasp 자체가 base+arm 동시 제어이기 때문에 전환 시점이 정밀하지 않아도 된다. Curriculum에서 0.7m~2.5m 범위로 학습하므로 그 안에만 들어오면 된다.
...
sim에서 전체 시스템을 돌린다는 것은: Isaac Sim이 환경을 렌더링하고, VLM(Qwen2.5-VL-7B)이 sim 카메라 이미지를 보고 instruction을 생성하고, VLA(π0-FAST)가 그 instruction + 이미지 + 9D state로 action을 생성하고, sim에서 action을 실행하는 전체 closed-loop이다. "빨간 컵 가져와"의 완전한 사이클을 sim에서 테스트한다.
...
**CarryAndPlace**: 목적지 부근에서 물체를 안전하게 놓기. 텔레옵 데이터 수집 시 **Combined 모드**를 사용하여 Skill-2(접근+파지) → Transit(미기록 이동) → Skill-3(목적지 접근+place)를 한 세션에서 3-phase 연속 레코딩할 수 있다. 실배포 시에는 Skill-2 → Skill-1(물체 든 채 Navigate) → Skill-3 순서로 VLM이 체이닝한다.
...
Critic은 Actor obs에 더해서 privileged 정보를 받는다. Skill-1 Critic은 25D(Actor 20D + speed/direction_compliance/closest_obs_dist/closest_obs_angle/time_remaining 5D), Skill-2 Critic은 37D(Actor 30D + bbox_full 3D + mass 1D + dist/heading/vel 3D), Skill-3 Critic은 36D(Actor 29D + bbox 3D + mass 1D + gripper_rel_pos 3D).
...
### 14-0. Phase 4.5 비교 실험: 우리 파이프라인 vs End-to-End VLA 베이스라인

3-Skill Hierarchical 파이프라인의 가치를 정량적으로 검증하기 위해, End-to-End VLA 베이스라인과 비교 실험을 수행한다.

**우리 파이프라인 (3-Skill Hierarchical)**:
```
VLM(Qwen2.5-VL-7B) → RL Navigate(Skill-1) → VLA Approach&Grasp(Skill-2) → VLA Carry&Place(Skill-3)
```
- Skill-1: Direction-conditioned RL (from scratch, sim only)
- Skill-2/3: 텔레옵 10~20개 → BC → RL → Expert Rollout → VLA fine-tuning
- 총 사람 데모: ~40개 (Skill-2 20 + Skill-3 20)
- sim DR로 강건성 확보, VLM+카탈로그로 새 물체/환경 적응

**베이스라인 (End-to-End VLA)**:
```
텔레옵 전체 task 데모 수집 → π0-FAST fine-tuning (skill 분리 없음, RL 없음)
```
- 사람이 전체 task("빨간 컵 가져와")를 처음부터 끝까지 시범
- 데모 수: 50~200개 (full task 길이)
- VLA 하나로 navigate + approach + grasp + carry + place 전부 처리
- VLM 오케스트레이션 없음

**비교 항목**:

| 항목 | 우리 파이프라인 | E2E 베이스라인 |
|------|---------------|--------------|
| 사람 데모 수 | ~40개 (skill별) | 50~200개 (full task) |
| 성공률 (sim) | 측정 예정 | 측정 예정 |
| 성공률 (real) | 측정 예정 | 측정 예정 |
| 새 물체 일반화 | VLM+카탈로그로 적응 | 학습 물체에 한정 |
| 새 환경 일반화 | sim DR + VLM 탐색 | 데모 환경에 한정 |
| 구현 복잡도 | 높음 (VLM+RL+VLA 3종) | 낮음 (VLA 1종) |
| sim 의존도 | 높음 (RL 학습, DR) | 낮음 (텔레옵만) |
| 학습 시간 | RL 학습 + VLA fine-tune | VLA fine-tune만 |

**예상 차별화 포인트**:
- 우리 파이프라인: 적은 데모, sim DR로 dynamics 강건성, VLM 상위 판단으로 실패 복구
- 베이스라인: 구현 단순, 학습 파이프라인 짧음, 디버깅 용이

**평가 프로토콜**: Phase 4.5에서 두 파이프라인을 동일 조건(동일 물체, 동일 환경, 동일 시작점)으로 각 30회+ sim 평가. 실패 유형 분류(navigation miss / grasp fail / drop / placement fail)까지 수행.
...
Stage A — Sim Full-System (3090 Desktop + A100 Server):
  C1~C7 전부 sim에서 평가 (각 30회+, Isaac Sim closed-loop)
...
Phase 4.5: Sim Full-System Evaluation (3090 Desktop + A100 서버)
  ...
  [⬜] VLM + VLA 통합 sim 평가 (전체 task "빨간 컵 가져와", 각 조건 30회+)
  [⬜] **비교 실험**: 우리 파이프라인(3-Skill) vs E2E VLA 베이스라인, 동일 조건 각 30회+
