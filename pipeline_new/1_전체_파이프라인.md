...
**gripper 표현**: RL/텔레옵/BC 전 파이프라인에서 **continuous position target**을 사용한다. binary 0/1로 바꾸면 그리퍼를 서서히 닫아가면서 물체를 감싸는 행동을 학습할 수 없다. `collect_demos.py`에서도 gripper 이진화를 제거하여 continuous 값을 그대로 HDF5에 저장한다. VLA 데이터 저장 시(`convert_hdf5_to_lerobot_v3.py`)에만 0.5 threshold로 binary 변환한다.
