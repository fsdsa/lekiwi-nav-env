...
**Skill별 RL obs가 다르다.** Skill-2(ApproachAndGrasp) Actor는 30D: 9D + base_vel(6) + arm_vel(6) + rel_object(3) + contact(2) + obj_bbox(3) + obj_category(1). Skill-3(CarryAndPlace) Actor는 29D: 9D + base_vel(6) + arm_vel(6) + home_rel(3) + grip_force(1) + obj_bbox(3) + obj_category(1). 두 skill 모두 물체 크기/종류(bbox, category)를 포함하여 12종 다중 물체에 대한 차별화된 전략을 학습한다. **속도 정보(base_vel 6D + arm_vel 6D = 12D)는 v8에서도 obs에 포함되어 있으며, 동적 제어에 필수다.** 이것을 빼면 Actor가 현재 운동 상태를 알 수 없어서 과도한 가감속, 오버슈트를 발생시킨다. 상세한 obs 구성은 Sim 데이터 수집 파이프라인 문서에서 다룬다.
...
### 4-2. VLM: SmolVLM-2 2.2B-Instruct

SmolVLM-2는 SigLIP vision encoder + SmolLM2 text decoder 구조의 2.2B 파라미터 VLM이다. A100에서 bf16 기준 ~5.2GB VRAM. Apache 2.0 라이선스.
...
**Navigate**: 목표 방향으로 이동하거나, 물체를 탐색하며 돌아다님. base만 움직이고 arm은 rest pose 유지.

**ApproachAndGrasp**: 물체에 base+arm 동시 접근하여 파지. 이동과 조작이 동시에 일어나는 가장 어려운 부분.
...
### 5-3. Navigate는 왜 RL을 하지 않는가

Navigate가 상대적으로 쉬운 문제다. holonomic base라서 아무 방향으로나 바로 갈 수 있다. Navigate RL을 위한 sim 환경 구축 비용(실내 공간 다양성)이 높고, 파이프라인에서 가장 어렵고 가치 있는 부분은 "이동하면서 동시에 잡기"다.

대신 Navigate 데이터는 sim에서 스크립트 정책(proportional controller)으로 자동 대량 생성한다. 사람 텔레옵이 필요 없다. 상세는 Sim 데이터 수집 파이프라인 문서에서 다룬다.

### 5-4. 통일된 9D Action

모든 skill의 데이터가 동일한 9D action으로 기록된다. Navigate에서도 arm 5D는 rest pose 유지 target, gripper는 open(1.0), base 3D는 이동 command로 9D action을 기록한다.
...
BC에서 핵심적인 설계 포인트: BC는 sim 안에서 실행하므로, 기록 시점에 9D state + sim ground truth(물체 상대 위치, 접촉 여부, 물체 크기/종류 등)를 함께 수집하여 RL Actor input과 동일한 형태를 만든다. **BC의 네트워크 구조(hidden dims, activation)가 RL Actor와 정확히 같아야 한다.** 기존 `models.py`의 `PolicyNet`을 공유한다. 물체 크기(obj_bbox)와 종류(obj_category)를 Actor에 포함시켜서 12종 다중 물체에 대해 물체별 차별화된 grasp/carry 전략을 학습할 수 있게 한다. **Orthogonal initialization (PPO 37 details)**: hidden layer는 gain=sqrt(2), policy output layer는 gain=0.01, value output layer는 gain=1.0으로 초기화한다. 이는 PPO 수렴 안정성을 높이는 표준 설정이며 `models.py`의 `PolicyNet`/`CriticNet` 모두에 적용한다.

기존 구현과의 관계: `train_bc.py`는 `--normalize` OFF가 기본(PPO RunningStandardScaler와 호환). BC checkpoint 구조는 `net.0/2/4.weight/bias + mean_layer.weight/bias` (log_std_parameter 없음). `train_lekiwi.py`가 BC weights를 PolicyNet에 로드하고 log_std_parameter를 보존한다.
...
Critic은 Actor obs에 더해서 물체 바운딩박스, 질량 등 더 많은 privileged 정보를 받는다. Skill-2 Critic은 37D(Actor 30D + bbox_full 3D + mass 1D + dist/heading/vel 3D), Skill-3 Critic은 36D(Actor 29D + bbox 3D + mass 1D + gripper_rel_pos 3D).
...
**PPO 핵심 하이퍼파라미터**: `ratio_clip=0.15` (기존 0.2에서 축소 — BC 초기화 후 빠른 policy 붕괴를 방지), `grad_norm_clip=0.5` (기존 1.0에서 축소 — 초기 high-gradient 구간의 안정성 확보). 나머지는 skrl 기본값 유지(lr=1e-4, mini_batches=8, epochs=8, lambda=0.95, gamma=0.99, entropy_coef=0.01).
...
Handoff Buffer의 내용(world-frame 절대 좌표 포함)은 sim 내부에서 reset 용도로만 쓰이고, VLA에 전달되는 데이터가 아니다. 최소 200~500개의 handoff entry를 확보한다.

**Handoff Noise Injection**: 버퍼 생성 시 각 entry에 노이즈를 추가한다 (arm joint ±0.05rad, object xy ±0.02m, base xy ±0.03m, base yaw ±0.1rad). 이유: 실제 배포 시 VLA의 Skill-2 출력은 RL Expert보다 부정확하다. Skill-3가 "완벽하지 않은 grasp 상태"에서도 복구/운반할 수 있도록 학습시켜야 한다. 노이즈 없이 완벽한 entry만 사용하면 Skill-3가 VLA의 실제 전환 상태를 한 번도 경험하지 못하여 배포 시 시작부터 실패한다.

### 7-3. Grasp 메커니즘

기존 v8 코드의 physics-based grasp를 유지한다. gripper가 close이고 물체와 접촉하면 `UsdPhysics.FixedJoint`로 결합한다.

**break_force 설정**: 기본값 `break_force = 30N`, 에피소드마다 DR로 15~45N 범위에서 랜덤화 (`dr_grasp_break_force_range`). 기존 v8의 `grasp_joint_break_force=1e8`(사실상 영구 결합)에서 변경한다. 이유: (1) 영구 결합이면 Skill-3(CarryAndPlace)에서 carry 중 조심하는 행동을 학습하지 않아서 real에서 물체를 떨어뜨린다. (2) 고정값이면 그 값에 과적합하므로 DR로 다양한 grasp 강도를 경험시킨다. 구현은 기존 v8의 FixedJoint attach/detach 패턴에 per-env break_force를 적용하는 형태다. **주의**: Skill-3 `_reset_idx()`에서 `_apply_domain_randomization()`을 `_attach_grasp_fixed_joint_for_envs()` **이전**에 호출해야 DR된 break_force가 joint에 반영된다.
...
**Action smoothness penalty (-0.005 × action_delta²)** — 실기 부드러운 동작. 연속 스텝 간 action 변화량 제곱에 비례한 페널티로, 급격한 관절 움직임과 기반부 방향 전환을 억제하여 실제 로봇의 서보 부하와 진동을 줄인다.
...
- Observation noise: joint_pos(0.01 rad), base_vel(0.02 m/s), object_rel(0.02 m) — 센서 노이즈 시뮬레이션
...
## 8. Navigate 데이터: 스크립트 정책으로 자동 생성

Navigate는 RL이 아니라 sim에서 스크립트 정책으로 데이터를 대량 생성한다. 사람 텔레옵이 필요 없다.

### 8-1. 두 가지 행동 패턴

**목표 방향 이동**: proportional controller로 trajectory 생성. target 방향으로 비례 제어.

**탐색 회전**: 제자리 회전 또는 전진하며 주변 탐색. 랜덤 방향으로 회전/전진하는 스크립트로 생성.

둘 다 arm은 REST_POSE 고정, gripper는 open(1.0) 고정. 9D action으로 기록.

### 8-2. 수집 방식

매 에피소드마다 시작 위치, 목표 위치, 물체 배치를 랜덤화하면서 스크립트 정책을 실행. (이미지, 9D state, 9D action, instruction)을 저장. 스크립트가 목표 좌표를 알고 있었다는 사실은 저장하지 않는다.

**노이즈 주입**: proportional controller 출력에 조향 흔들림(σ=0.05 rad), 속도 양자화(3~5단계), 간헐적 지연(5% 확률 1~3프레임 이전 action 반복).
...
### 12-3. Joint Limits — JSON 생성 ✅, USD PhysX 반영 ✅ (코드에서 `arm_limit_write_to_sim=True`)

`build_arm_limits_real2sim.py`로 실측 joint range에서 `calibration/arm_limits_real2sim.json`을 생성 완료. 이 값은 RL 학습 시 **제어 target clamp와 USD PhysX joint limit 양쪽 모두에 적용**한다 (`arm_limit_write_to_sim=True`). USD의 joint limit이 inf로 설정된 경우 팔이 몸체를 관통하는 문제가 발생하므로, 물리 엔진 레벨에서 캘리브레이션된 실제 관절 범위를 강제한다.
...
### 12-4. REST_POSE — 기존 값 존재, 안전 자세 재정의 필요 ⬜

현재 `leader_to_home_tcp_rest_matched_with_keyboard_base.py`에 `SIM_REST_RAD6`이 정의되어 있으나, 이 값은 거의 all-zeros(팔이 완전히 펴진 상태)다. all-zeros는 토크가 크고 위험하므로, 팔이 접힌 안전한 tucked pose를 새로 정의해야 한다. 이 값은 Navigate skill에서 arm 고정 target, RL 초기 arm 자세로 사용된다.
...
A100: SmolVLM-2가 instruction 생성, VLA가 action chunk 생성, Jetson으로 반환. Jetson은 sim→real 역변환 후 모터에 명령.
...
7개 조건. 모든 조건에서 orchestrator VLM은 SmolVLM-2 고정.
...
- conda inference: SmolVLM-2 추론
- VRAM budget: SmolVLM-2 ~5GB + VLA 추론 ~8GB = ~13GB
...
Phase 0: Sim-Real 일치 ★ Phase 1 시작 전 필수 (Hard Gate) ★
  ...
  [✅] Arm joint range 실측 (arm_limits_real2sim.json 생성)
  ...
  [✅] Joint limits USD PhysX 반영 → `arm_limit_write_to_sim=True`로 코드에서 자동 적용
  [⬜] REST_POSE 안전 자세 재정의 (현재 SIM_REST_RAD6 ≈ all-zeros)
  [✅] 데이터셋 형식 확인 — yubinnn11/lekiwi3 (v3.0), velocity(m/s, rad/s)

Phase 1: RL Expert 학습 (RTX 3090)
  [✅] v8 단일 환경 → Skill-2/3 분리 환경 리팩토링 (lekiwi_skill2_env.py, lekiwi_skill3_env.py + AAC 3파일)
  [⬜] Skill-2 텔레옵 10~20개
  [⬜] Skill-2 BC 학습 (30D obs: 9D + vel 12D + rel_object + contact + bbox + category)
  [⬜] Skill-2 RL (BC→PPO, 성공률 90%+)
  [⬜] Handoff Buffer 생성 (200~500개)
  [⬜] Skill-3 텔레옵 10~20개 (Handoff Buffer 상태에서)
  [⬜] Skill-3 BC 학습 (29D obs: 9D + vel 12D + home_rel + grip_force + bbox + category)
  [⬜] Skill-3 RL (BC→PPO, 성공률 90%+)

Phase 2: VLA Training Data 수집 (RTX 3090)
  [⬜] Navigate 스크립트 정책 데이터 자동 생성 (1K~2K개)
