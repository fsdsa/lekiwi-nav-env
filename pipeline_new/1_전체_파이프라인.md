...
**Navigate 보상 구조 (v8: action masking + simple tracking)**: reward-based drift 억제(v6f~v7)가 4회 실패 후, **action masking**으로 구조적 해결. `_apply_action()`에서 명령별 불필요한 action dim을 0으로 강제하여 drift를 원천 차단한다. fwd/bwd→vx,wz=0, strafe→vy,wz=0, turn→vx,vy=0. 보상은 v6c 스타일 단순 구조: (1) linear tracking(1.5, std=0.25): `exp(-||target_lin - actual_lin||² / 0.25²)`, (2) angular tracking(1.5, std=0.25): `exp(-(target_wz - actual_wz)² / 0.25²)`, (3) smoothness(-0.005): masked action delta² penalty, (4) time(-0.01): per-step cost. collision/proximity reward 제거. **direction_cmd turn magnitude**: ±0.33 (target wz = 1.0 rad/s). **Sample efficiency**: action masking으로 명령당 gradient 비율이 1/3이므로 수렴에 ~3배 iteration 필요 (masking 30K ≈ 원래 10K). Early stop: `rew_track_ang >= 1.35 AND ang_vel_error <= 0.02 AND avg_speed >= 0.25 AND direction_compliance >= 0.55`.

**Navigate v8 학습 (2026-02-25 진행중)**:
- **v8 (action masking)**: reward 경쟁 문제를 구조적으로 해결. A100 서버에서 216K iterations 학습 진행중.
- **이전 시도 (v6f~v7, 모두 폐기)**: v6f(turn_drift penalty), v7(along/ortho split std=0.05/0.10), v6g(ortho drift penalty) — 모두 reward 경쟁에서 tracking이 drift penalty를 압도하여 실패.
- **기존 백업**: `best_agent_v6e.pt`, `best_agent_v6g2.pt`, `best_agent_v6g3.pt`
...
Phase 1: RL Expert 학습 (텔레옵: 3090 Desktop, BC/RL: A100 서버)
  ...
  [🔄] Skill-1 RL v8 (action masking) 학습 진행 중 (A100, 8192 envs, 216K iterations)
        - v8: action masking으로 drift 구조적 제거 + simple tracking reward (lin/ang std=0.25)
        - Early stop: rew_track_ang>=1.35 AND ang_vel_error<=0.02 AND avg_speed>=0.25 AND direction_compliance>=0.55
        - 이전 시도 (v6f~v7, v6g 모두 폐기): reward 경쟁 문제로 실패
