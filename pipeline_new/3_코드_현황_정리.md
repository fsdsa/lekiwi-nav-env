...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `lekiwi_skill1_env.py` | ~1265 | Skill-1 Navigate RL 환경 (Direction-conditioned: 방향 명령 추종 + 장애물 회피 + pseudo-lidar, 20D actor / 25D critic). **서버 적용**: `activate_contact_sensors=False`, `PhysxCfg(gpu_max_rigid_patch_count=2**18)` (8192 envs 대응) |
...
```
Phase 1 (Skill-2 기록):
  매 스텝 env._compute_skill2_actor_obs()로 30D obs 기록
...
Navigate는 `train_lekiwi.py`에서 `--skill navigate` 시 자동으로 `EarlyStopCfg(metric="direction_compliance")` 활성화.
수동 설정: `--early_stop_metric <key> --early_stop_threshold 0.93 --early_stop_window 500`.
