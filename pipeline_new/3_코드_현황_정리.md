...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `lekiwi_skill1_env.py` | ~1258 | Skill-1 Navigate RL 환경 (Direction-conditioned: 방향 명령 추종 + pseudo-lidar, 20D actor / 25D critic). **현재 코드: v6g2** (`ortho_drift_weight=-1.0`, linear penalty). v6g2_full 서버 학습 진행중. **v6g3** (`ortho=-15.0`, quadratic) 완주됨→backup. collision/proximity reward 제거, 장애물 스폰은 유지(시각적 다양성). `ortho_speed` TensorBoard 로그. **서버 적용**: `activate_contact_sensors=False`, `PhysxCfg(gpu_max_rigid_patch_count=2**18)` (8192 envs 대응) |
...
| AUDIT-V2-18 | High | Navigate v6g→v7→v8: reward-based drift 억제 4회 실패(v6f/v6g/v7/v7b) → **v8: action masking**으로 구조적 해결. `_apply_action()`에서 명령별 불필요 dim=0 강제. simple tracking reward (lin/ang std=0.25). Early stop 4조건 | `lekiwi_skill1_env.py`, `aac_trainer.py`, `train_lekiwi.py` | 진행 중 (v8, 216K iter 학습중) |
...
| (학습중) v8 | v8 | 서버 216K iter 학습중 | action masking + simple tracking, 현재 코드 상태 |

**v8 접근**: reward-based drift 억제(v6f/v6g/v7) 4회 실패 → action masking으로 구조적 해결. 명령별 불필요 action dim을 `_apply_action()`에서 0으로 강제. sample efficiency 1/3 (6개 명령 균등 분배)이므로 수렴에 ~3배 iteration 필요.
...
- [🔄] Skill-1 v8 (action masking) 학습 진행중 (A100, 216K iter) — drift 구조적 해결
