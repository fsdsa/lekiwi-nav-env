...
@dataclass
class EarlyStopCfg:
    metric: str = ""          # extras["log"] 키 (예: "direction_compliance")
    threshold: float = 0.93   # rolling avg 이 값 이상이면 수렴
    window: int = 500         # rolling average 윈도우 크기
    min_timesteps: int = 2000 # 최소 학습 timestep

class AACSequentialTrainer(SequentialTrainer):
    def __init__(self, *args, early_stop: EarlyStopCfg | None = None, **kwargs):
        ...
        self._es_buffer = deque(maxlen=early_stop.window)
    ...
    def single_agent_train(self):
        ...
        for timestep in range(...):
            ...
            # Early stopping: rolling avg check
            if es and len(self._es_buffer) >= es.window:
                avg = sum(self._es_buffer) / len(self._es_buffer)
                if avg >= es.threshold:
                    return  # 수렴 → 학습 중단
```

Navigate는 `train_lekiwi.py`에서 `--skill navigate` 시 자동으로 `EarlyStopCfg(metric="rew_track_ang", threshold=1.35, metric2="ang_vel_error", threshold2=0.02)` 활성화.
...
| AUDIT-V2-18 | High | Navigate v6g: v6f base + ortho drift penalty. 비교 실험 진행중: v6g2(ortho=-1.0, linear, 코드 현재 상태) vs v6g3(ortho=-15.0, quadratic, 완주→backup). 전후진 drift 해결이 핵심 | `lekiwi_skill1_env.py` | 진행 중 (v6g2_full 학습중, v6g3 완주) |
...
## 6. Navigate 체크포인트 백업 현황 (2026-02-24)

| 파일 | 버전 | 상태 | 비고 |
|------|------|------|------|
| `best_agent_v6e.pt` | v6e | 완주 | collision/proximity 제거, 순수 velocity tracker |
| `best_agent_v6g2.pt` | v6g2 | 17K steps 중간 | ortho=-1.0, linear penalty |
| `best_agent_v6g3.pt` | v6g3 | 완주 | ortho=-15.0, quadratic penalty |
| (학습중) v6g2_full | v6g2 | 서버 학습 진행중 | 처음부터 완주 예정, 현재 코드 상태 |

**비교 실험 목적**: 전후진 시 뒷바퀴 drift 억제를 위한 ortho penalty 최적 설정 탐색. v6g2_full 완주 후 v6g3과 TensorBoard 메트릭(`ortho_speed`, `lin_vel_error`, `direction_compliance`) 비교하여 최종 버전 선정.
...
- [🔄] Skill-1 v6g 비교 실험 (v6g2_full 학습중, v6g3 완주) — 전후진 drift 해결 필요
