...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `lekiwi_skill1_env.py` | ~1260 | Skill-1 Navigate RL 환경 (Direction-conditioned: 방향 명령 추종 + pseudo-lidar, 20D actor / 25D critic). **현재 코드: v6c** (simple tracking + collision/proximity, action masking 없음). v6c vs v6f 비교 후 최종 선택. **Inference-time masking**: eval/deployment에서만 cross-axis 잔차 제거 (`eval_navigate.py`, `collect_demos.py`). **서버 적용**: `activate_contact_sensors=False`, `PhysxCfg(gpu_max_rigid_patch_count=2**18)` (8192 envs 대응) |
| `lekiwi_skill2_env.py` | ~1700 | Skill-2 ApproachAndGrasp 환경. **Reward 개선**: wider gripper shaping(0.35~0.70m도 0.3×close_progress 보상), height penalty(-0.1×clamp(\|z_offset\|-0.10, min=0), 위에서 접근 방지). **Dest object background 스폰**: `--dest_object_usd`로 빨간 컵을 background prop으로 스폰, Skill-3가 상속 |
| `lekiwi_skill3_env.py` | ~580 | Skill-3 CarryAndPlace 환경 (**destination object 목적지**: `dest_object_pos_w` 랜덤 스폰, `_check_place_success()` 3D 판정, intentional place + CRITICAL-2 fix + batched reset + **combined mode** + `_compute_skill2_actor_obs`) |
| `generate_handoff_buffer.py` | 176 | Handoff Buffer 생성기 (AAC 지원) |
| `collect_navigate_data.py` | 601 | Skill-1 Navigate 데이터 수집 (스크립트 정책, fallback) |
...
| `aac_trainer.py` | 146 | AAC trainer — env.state()로 critic obs 추적 + EarlyStopCfg (rolling avg early stopping) |
| `eval_bc.py` | ~297 | BC 체크포인트 Isaac Sim GUI 평가. **GMM/MSE 자동 감지** (`pi_layer` 키 존재 여부), HDF5 초기 상태 복원, action_repeat, arm_action_scale |
| `eval_full_system.py` | — | Sim Full-System 평가 (Phase 4.5): VLM+VLA closed-loop sim 검증, relative placement 기반 성공 판정 |
| `vlm_orchestrator.py` | ~280 | VLM 오케스트레이터: RelativePlacementOrchestrator, classify_user_request(VLM `/classify` 호출). **Task 분류는 VLM 텍스트 호출** — 키워드 매칭 제거, Qwen2.5-VL이 relative_placement + grasp_object/dest_object 추출 |
| `vlm_prompts.py` | ~106 | VLM System Prompt 상수: RELATIVE_PLACEMENT_SYSTEM_PROMPT, NAVIGATE_SYSTEM_PROMPT |
...
| `train_lekiwi.py` | ~540 | `--skill` 분기 (navigate/approach_and_grasp/carry_and_place), BC warm-start, **AAC 통합**, Navigate PPO config |
| `train_bc.py` | 277 | `--expected_obs_dim` 필수, v6 action 이름 수정, **GMM loss 기본** (`--loss gmm --n_components 5`), `bc_nav.pt`(RL warm-start용) + `bc_nav_gmm.pt`(full GMM) 이중 저장 |
| `collect_demos.py` | ~1110 | `--skill` 분기 (navigate 추가), gripper continuous, **Skill1/2/3EnvWithCam 추가**, Navigate action override, **inference-time masking** |
| `convert_hdf5_to_lerobot_v3.py` | ~530 | `convert_to_vla_units()` passthrough, `infer_robot_state_from_obs()` dim==20 추가 |
| `record_teleop.py` | ~960 | `--skill` 분기 (+ **combined** 추가), v6 action format, `arm_limit_write_to_sim=False`, wrist_roll 디버그 출력, **combined 듀얼 HDF5 + phase-aware 레코딩**, **`--resume` 이어서 녹화**, **`--dest_object_usd`** (빨간 컵 background 스폰), contact sensor 기반 hold 판정 |
...
class Skill2EnvCfg(DirectRLEnvCfg):
    ...
    object_dist_min: float = 0.7
    object_dist_max: float = 0.9    # LeKiwi 소형 로봇, arm reach ~0.4m 고려
    ...
    curriculum_current_max_dist: float = 0.7
...
### 3-8. collect_navigate_data.py (601줄, fallback)
...
### 3-10. record_teleop.py (~930줄)

텔레옵 데모 녹화. ROS2/TCP 듀얼 입력, `--skill` 분기 지원.

> 전체 코드: `record_teleop.py` 파일 직접 참조 (~930줄).
...
`--skill combined` 시 Skill3Env를 `_combined_mode=True`로 생성하고, `grasp_gripper_threshold=0.5`으로 설정.
...
# ...
if is_combined:
    env_cfg.grasp_gripper_threshold = 0.5  # 확실한 파지만 인정
