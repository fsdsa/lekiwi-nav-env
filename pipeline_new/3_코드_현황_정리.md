...
| Curriculum | 시작 0.7m → 성공률 70% 초과 시 0.25m 증가 |
...
| `lekiwi_skill2_env.py` | 1647 | Skill-2 ApproachAndGrasp 환경. **Reward 개선**: wider gripper shaping(0.35~0.70m도 0.3×close_progress 보상), height penalty(-0.1×clamp(\|z_offset\|-0.10, min=0), 위에서 접근 방지) |
| `lekiwi_skill3_env.py` | ~530 | Skill-3 CarryAndPlace 환경 (intentional place + CRITICAL-2 fix + batched reset + **combined mode** + `_compute_skill2_actor_obs`) |
...
| `eval_full_system.py` | — | Sim Full-System 평가 (Phase 4.5): VLM+VLA closed-loop sim 검증 |
| `vlm_inference_server.py` | — | VLM 추론 서버 (A100, Qwen2.5-VL-7B, HTTP API) |
...
| `collect_demos.py` | ~1110 | `--skill` 분기 (navigate 추가), gripper binary, **Skill1/2/3EnvWithCam 추가**, Navigate action override |
| `convert_hdf5_to_lerobot_v3.py` | ~530 | `convert_to_vla_units()` passthrough, `infer_robot_state_from_obs()` dim==20 추가 |
| `record_teleop.py` | ~930 | `--skill` 분기 (+ **combined** 추가), v6 action format, `arm_limit_write_to_sim=False`, wrist_roll 디버그 출력, **combined 듀얼 HDF5 + phase-aware 레코딩** |
...
def _compute_skill2_actor_obs(self) -> torch.Tensor:
    ...
    metrics = self._compute_metrics()
    # ... Skill-2의 object_pos_b, contact_lr 포함, home_rel/grip_force 미포함
...
- `--grasp_hold_steps`: combined 모드 전용, Phase 1→2 전환 파지 유지 스텝 수 (기본 600 = 10s @ 60Hz)
- `--home_dist_thresh`: combined 모드 전용, Phase 2→3 전환 거리 (기본 0.7m)
...
Phase 1 (Skill-2 기록):
  ...
  object_grasped 연속 체크 → grasp_hold_steps 도달 시:
    → Skill-2 에피소드 저장 → Phase 2 전환
    → episode_length_buf = 0 (Transit 타이머 리셋)

Phase 2 (Transit, 미기록):
  매 스텝 episode_length_buf = 0 (timeout 방지)
  사람이 키보드로 home 근처까지 이동
  home_dist < 0.7m AND |heading_to_home| < 0.76rad 시:
    → Phase 3 전환
...
**Phase 2→3 전환 조건 (Skill-1→Skill-2 전환과 동일 로직):**
```python
home_rel_b = quat_apply_inverse(root_quat, home_delta_w)
home_dist = torch.norm(home_rel_b[:2]).item()
heading_to_home = math.atan2(home_rel_b[0], home_rel_b[1])  # +y forward
transition = home_dist < 0.7 and abs(heading_to_home) < 0.76
```
...
# Phase 1→2: 물체를 잡고 10초(600스텝) 유지
# Phase 2→3: home이 0.7m 이내 + FOV 내
...
**목적**: 3-Skill Hierarchical 파이프라인의 가치를 End-to-End VLA 베이스라인과 정량 비교

| | 우리 파이프라인 (3-Skill) | 베이스라인 (E2E VLA) |
|---|---|---|
| 구조 | VLM → RL Navigate → VLA Approach&Grasp → VLA Carry&Place | 텔레옵 전체 task → π0-FAST fine-tuning |
| 사람 데모 | ~40개 (skill별 10~20) | 50~200개 (full task) |
| RL 사용 | Skill-1 (Navigate) | 없음 |
| Skill 분리 | 3 skills | 단일 정책 |
| VLM 오케스트레이션 | Qwen2.5-VL-7B | 없음 |
| 새 물체 적응 | VLM+카탈로그 | 재학습 필요 |
...
- [ ] **비교 실험**: 우리 파이프라인 vs E2E VLA 베이스라인 (Phase 4.5)
