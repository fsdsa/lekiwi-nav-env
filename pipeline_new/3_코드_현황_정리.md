...
| Place 목적지 | Skill-3: `dest_object_pos_w` 랜덤 스폰(2.0~4.0m from origin, 물체와 최소 0.5m 분리), `_check_place_success()` = XY radius(0.20m) + Z height tolerance(0.10m) + gripper open + not dropped. `home_pos_w`는 out_of_bounds 판정용으로만 유지 |
...
**Config 수정 (2026-02-26)**: `place_gripper_threshold: 0.6` (기존 0.3, grip=0.34에서 의도치 않은 place 트리거 방지), `dest_spawn_dist: 2.0~4.0m` (기존 2.0~3.0m), `near_dest: place_radius×3.0` (기존 `dest_spawn_dist_max` — 항상 true 버그 수정).
...
class Skill3EnvCfg(Skill2EnvCfg):
    ...
    dest_spawn_dist_min: float = 2.0  # env origin으로부터 최소 거리
    dest_spawn_dist_max: float = 4.0  # env origin으로부터 최대 거리
...
class Skill3Env(Skill2Env):
    ...
    def _reset_from_handoff(self, env_ids: torch.Tensor, num: int):
        ...
        # dest object 랜덤 스폰 (env origin 근처 2.0~4.0m, grasp 물체와 최소 0.5m 분리)
