  ...
  record_teleop.py          collect_demos.py        collect_navigate_data.py
       ...
       └────────┬────────────────┘                         │
           ...
           HDF5 (obs, actions, robot_state, images)        │
                │                                          │
           train_bc.py (BC 학습, GMM 기본, obs_dim 필수)    │
                │                                          │
           bc_nav.pt + bc_nav_gmm.pt                       │
                │                                          │
           bc_nav.pt ──▶ train_lekiwi.py (PPO + BC warm-start)
                         --skill navigate/approach_and_grasp/carry_and_place
                                │
                         best_agent.pt
                                │
                    collect_demos.py (expert rollout, --skill 분기)
                                │
                   HDF5 + camera images
                                │
...
| Grasp break | Skill-2 drop detection 제거 (정상 파지 오판 문제). `just_dropped` 필드는 Skill-3용으로 유지 |
...
| Intentional Place | Skill-3: gripper_open(>`place_gripper_threshold` 0.6) + near_dest(`place_radius×3.0` = 0.6m 이내) → FixedJoint 해제 (just_dropped=False 유지, place_success 달성 가능) |
...
| `lekiwi_skill2_env.py` | ~1876 | Skill-2 ApproachAndGrasp 환경. **Reward 개선**: wider gripper shaping(0.35~0.70m도 0.3×close_progress 보상), height penalty(-0.1×clamp(\|z_offset\|-0.10, min=0), 위에서 접근 방지). **Dest object background 스폰**: `--dest_object_usd`로 빨간 컵을 background prop으로 스폰, Skill-3가 상속 |
...
| `record_teleop.py` | ~1265 | `--skill` 분기 (+ **combined** 추가), v6 action format, `arm_limit_write_to_sim=False`, wrist_roll 디버그 출력, **combined 듀얼 HDF5 + phase-aware 레코딩**, **`--resume` 이어서 녹화**, **`--dest_object_usd`** (빨간 컵 background 스폰), contact sensor 기반 hold 판정 |
...
### 3-2. lekiwi_skill2_env.py (1876줄)
...
- drop detection 제거 — 정상 파지 오판 문제 (gripper body~물체 중심 자연 오프셋 ~0.18m)
...
- `_update_grasp_state` (1249-1314): contact 기반 grasp (drop detection 제거됨)
