...
| VLM 상위 플래너 | VLM(Qwen3-VL-8B)이 2가지 역할: ① `/classify`(텍스트 only, 1회) ��� 사용자 명령 → relative_placement 분류 + grasp_object/dest_object 추���, ② `/infer`(이미지+텍스트, 0.3Hz 반복) — 상황판단 + phase ��환 + VLA instruction 생성. 키���드 매칭 제거, JSON 파싱 실패 시 fallback |
...
| `vlm_orchestrator.py` | ~128 | VLM 오케스트레이터: RelativePlacementOrchestrator, classify_user_request(VLM `/classify` 호출). **Task 분류는 VLM 텍스트 호출** — 키워드 매칭 제거, Qwen3-VL이 relative_placement + grasp_object/dest_object 추출 |
| `vlm_prompts.py` | ~72 | VLM System Prompt 상수: RELATIVE_PLACEMENT_SYSTEM_PROMPT, NAVIGATE_SYSTEM_PROMPT |
| `vlm_inference_server.py` | — | VLM 추론 서버 (A100, Qwen3-VL-8B, HTTP API). **2개 엔드포인트**: `/infer`(이미지+텍스트 → instruction+phase, 실시�� 오케스트레이션), `/classify`(텍스트 only → mode+target_object, task 시작 시 1회) |
...
- [ ] vlm_inference_server.py 구현 (Qwen3-VL-8B HTTP API)
