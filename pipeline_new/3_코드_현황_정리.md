...
>
> **서버(A100) 설치/전송/학습 가이드**: `pipeline_new/server_guide.md` 참조
...
│  (RL: AAC-PPO)         (RL: AAC-PPO)              (RL: AAC-PPO)     │
│  lekiwi_skill1_env.py  lekiwi_skill2_env.py        lekiwi_skill3_env.py  │
│  20D actor / 25D critic 30D actor / 37D critic     29D actor / 36D critic│
...
```
텔레옵(리더암+키보드)          RL Expert              Script Policy (fallback)
       │                         │                         │
  record_teleop.py          collect_demos.py        collect_navigate_data.py
       ...
       └────────┬────────────────┘                         │
         ...
         ┌──────┴──────────────────┬──────────────────┐      │
         ▼                       ▼                  ▼      │
    train_bc.py          train_act_bc.py   train_diffusion  │
    (MLP, GMM)           (ACT, CVAE+L1)    _bc.py (DP,DDPM)│
         │                       │                  │      │
    bc_nav.pt            act_best.pt        dp_bc.pt       │
         │                       │                  │      │
    train_lekiwi.py      train_act_rl.py   train_resip.py  │
    (PPO+AAC)            (Chunk PPO)       (Residual PPO)  │
         │                       │                  │      │
    best_agent.pt        act_rl_best.pt    resip_best.pt   │
         │                       │                  │      │
    collect_demos.py     generate_expert   collect_resip   │
         │               _data.py          _demos.py       │
         └──────┬────────────────┴──────────────────┘      │
                ▼                                          │
           HDF5 + camera images                            │
          ...
          convert_hdf5_to_lerobot_v3.py                    │
           ...
           LeRobot v3 dataset → VLA 파인튜닝               │
                ...
                ┌───────────┴──────────────┐               │
                ▼                          ▼               │
        eval_full_system.py          (Phase 5: 실배포)     │
        (Sim Full-System 검증)                             │
        VLM(A100) + VLA(A100)                              │
        + Isaac Sim(3090 Desktop)                          │
```
...
action[6:8]  = base linear velocity vx, vy (2D)  ← body frame
action[8]    = base angular velocity wz (1D)      ← body frame
```

**Action 좌표계**: action[6:9]는 **body frame** (vx=right, vy=forward, wz>0=CCW per Isaac Sim right-hand rule). `_apply_action()` 내부에서 body→IK 변환을 수행한다:
```python
ik_vx = body_vy       # IK vx+ = body vy+ (forward)
ik_vy = -body_vx      # IK vy+ = body vx- (left)
ik_wz = body_wz       # IK wz+ = CCW = body wz+ (same convention)
```
IK와 Isaac Sim 모두 CCW+ 규약이므로 wz는 부호 반전 없이 그대로 전달한다. 이로써 obs(base_body_vel, direction_cmd)와 action이 동일한 body frame이 되어 `action ∝ direction_cmd` 관계가 성립한다.
...
| Curriculum | 시작 0.8~1.2m 전체 범위 랜덤 (curriculum 사실상 비활성, 성공률 70% 초과 시 0.25m 증가 로직은 유지) |
| Grasp | **순수 마찰** (FixedJoint 완전 제거), EE offset 기반 판정 (`grasp_ee_max_dist=0.07`) |
| Gripper | 전 파이프라인 continuous (RL/BC/텔레옵/VLA 모두 연속값, 이진화 제거) |
| Base 단위 | sim=m/s, VLA=m/s (변환 불필요) |
| Handoff | Skill-2 성공 상태 → pickle → Skill-3 초기 상태 |
| Gripper body | `"Moving_Jaw_08d_v1"` via `find_bodies()` |
| Drop detection | Skill-2 env에서 제거 → **train_resip.py reward-level**에서 구현 (v7.1): `ms_gr & (oh<0.04) & (ee_3d>0.15)` → R3 회수, milestone 리셋 |
| AAC | skrl 1.4.3 네이티브 미지원 → `aac_wrapper.py` + `aac_ppo.py` + `aac_trainer.py` 3파일로 구현 |
| Place 목적지 | Skill-3: `dest_object_pos_w` 랜덤 스폰(4.0~5.0m from origin, 물체와 최소 0.5m 분리), `_check_place_success()` = XY radius(0.20m) + Z height tolerance(0.10m) + gripper open + not dropped. `home_pos_w`는 out_of_bounds 판정용으로만 유지 |
| Intentional Place | Skill-3: gripper_open(>`place_gripper_threshold` 0.6) + near_dest(`place_radius×3.0` = 0.6m 이내) → just_dropped=False 유지, place_success 달성 가능 |
| VLM 상위 플래너 | VLM(Qwen2.5-VL-7B)이 2가지 역할: ① `/classify`(텍스트 only, 1회) — 사용자 명령 → relative_placement 분류 + grasp_object/dest_object 추출, ② `/infer`(이미지+텍스트, 0.3Hz 반복) — 상황판단 + phase 전환 + VLA instruction 생성. 키워드 매칭 제거, JSON 파싱 실패 시 fallback |
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `lekiwi_skill1_env.py` | ~1270 | Skill-1 Navigate RL 환경 (Direction-conditioned: 방향 명령 추종 + pseudo-lidar, 20D actor / 25D critic). **현재 코드: v6c** (simple tracking + collision/proximity, action masking 없음). v6c vs v6f 비교 후 최종 선택. **Inference-time masking**: eval/deployment에서만 cross-axis 잔차 제거 (`eval_navigate.py`, `collect_demos.py`). **서버 적용**: `activate_contact_sensors=False`, `PhysxCfg(gpu_max_rigid_patch_count=2**18)` (8192 envs 대응) |
| `lekiwi_skill2_env.py` | ~1750 | Skill-2 ApproachAndGrasp 환경. **FixedJoint 완전 제거** → 순수 마찰 grasp. **EE offset 기반 grasp 판정** (`Wrist_Roll_08c_v1` + local offset, `grasp_ee_max_dist=0.07`). **bbox center 보정** (USD origin→geometric center, `quat_apply`). **Ground contact sensor**: CuboidCfg ground + **gripper+wrist 2개 센서**. **Dest object background 스폰**: `--dest_object_usd`로 빨간 컵 스폰, Skill-3 상속. `grasp_contact_threshold=0.7`, `lift_contact_threshold=0.55` |
| `lekiwi_skill3_env.py` | ~622 | Skill-3 CarryAndPlace 환경 (**destination object 목적지**: `dest_object_pos_w` 랜덤 스폰, `_check_place_success()` 3D 판정, intentional place + CRITICAL-2 fix + batched reset + **combined mode** + `_compute_skill2_actor_obs`) |
| `generate_handoff_buffer.py` | 178 | Handoff Buffer 생성기 (AAC 지원) |
| `collect_navigate_data.py` | 600 | Skill-1 Navigate 데이터 수집 (스크립트 정책, fallback) |
...
| `aac_ppo.py` | 378 | AAC PPO agent — critic에 별도 privileged obs 전달 |
| `aac_trainer.py` | 188 | AAC trainer — env.state()로 critic obs 추적 + EarlyStopCfg (rolling avg early stopping) |
| `eval_bc.py` | ~296 | BC 체크포인트 Isaac Sim GUI 평가. **GMM/MSE 자동 감지** (`pi_layer` 키 존재 여부), HDF5 초기 상태 복원, action_repeat, arm_action_scale |
| `eval_full_system.py` | — | Sim Full-System 평가 (Phase 4.5): VLM+VLA closed-loop sim 검증, relative placement 기반 성공 판정 |
| `act_fork/` | — | **공식 ACT fork** (tonyzhaozh/act). DETR-VAE state-only, 아래 파일 구조 참조 |
| `act_fork/detr/models/detr_vae.py` | ~180 | DETRVAE 코어. state_dim/action_dim 파라미터화, state-only 경로: [latent, state] 2-token → Transformer Decoder (7L, 8H). latent_dim=32, hidden_dim=256 |
| `act_fork/detr/models/transformer.py` | — | DETR Transformer encoder/decoder (공식 그대로) |
| `act_fork/detr/main.py` | ~80 | `build_ACT_model_and_optimizer()` — dict config 기반, argparse 충돌 없음 |
| `act_fork/policy.py` | ~60 | ACTPolicy wrapper. L1(masked by is_pad) + kl_weight×KL loss, state-only 시 image normalization 스킵 |
| `act_fork/utils.py` | ~200 | EpisodicDataset (HDF5→에피소드별 random start + action chunk padding) + **per-dim normalization** (mean/std, std clip 1e-2) |
| `act_fork/imitate_episodes.py` | ~250 | ACT BC 학습. AdamW(lr=1e-4, wd=1e-4), AMP bf16, val-first, best/last/periodic 체크포인트. `--demo_dir --skill --chunk_size 20 --kl_weight 10 --num_epochs 3000` |
| `act_fork/eval_act_bc.py` | ~160 | ACTInference: normalize → forward(z=0) → denormalize + **temporal ensembling** (exp(-0.01×k) weighted). `dataset_stats.pkl`에서 norm stats 로드 |
| `act_fork/test_sanity.py` | ~120 | 모델 빌드/학습/추론/AMP 검증 테스트 |
| `act_model.py` | ~350 | (이전 자체 구현, act_fork로 대체됨. 서버 학습 중인 실험 v1/v2용으로 유지) |
| `train_act_bc.py` | ~280 | (이전 자체 구현, act_fork/imitate_episodes.py로 대체됨. 서버 학습 중인 실험용으로 유지) |
| `train_act_rl.py` | ~350 | ACT-RL Chunk-level PPO. ACT backbone FROZEN + 학습 파라미터: log_std(K×9) + Value MLP(~130K). Chunk discount δ=Σγⁱr+γᴷV-V, KL anchor β×KL(π‖π_ACT). `--act_checkpoint`으로 BC 체크포인트 로드 |
| `generate_expert_data.py` | ~200 | ACT-RL expert rollout → HDF5 저장. deterministic mean 사용 (z=0, temporal ensembling). `--act_rl_checkpoint`으로 학습된 ACT-RL 체크포인트 로드 |
| `diffusion_policy.py` | ~694 | Diffusion Policy + Residual Policy (공식 ResiP 코드 포팅). **ConditionalUnet1D**(공식 그대로, down_dims 설정 가능 — v1=[256,512,1024] 65M 폐기, **v2=[64,128,256] ~5.3M 채택**), **LinearNormalizer**(min-max [-1,1]), **SwitchEMA**(공식 그대로), **ResidualPolicy**(actor+critic+logstd, 공식 그대로), **DiffusionPolicyAgent**(DDPM 학습 + DDIM 16-step 추론 + warm-start). 의존: `diffusers` (DDPMScheduler/DDIMScheduler) |
| `train_diffusion_bc.py` | ~471 | Diffusion Policy BC 학습 (공식 `src/train/bc.py` 적응). DiffusionPolicyDataset(HDF5→(obs, action_seq) 쌍, pred_horizon=16), AdamW + CosineAnnealing + EMA + gradient clipping. `--demo_path <HDF5> --obs_dim 30 --epochs 500 --eval`. 출력: `checkpoints/dp_bc.pt` |
| `train_resip.py` | ~830 | Residual PPO 학습 (**Reward v7.1b**, Direct Reward, **9개 항목 + drop detection**). Frozen DP + ResidualPolicy(~154K), `per_dim_action_scale`(arm=0.20, grip=0.30, base=0.35). PPO: actor lr=1e-3, critic lr=5e-3, target_kl=1.5, clip=0.2, ent_coef=0.001, **rew_norm=OFF**. Reward: R1(gripper open+10) + **R2(EE proximity×3, tanh σ=0.20, cumulative budget=80)** + R3(verified grasp+100, 5-step) + R4(lift×200, ee_3d<0.20) + R4b(lifted pose×30, σ=2.0) + R5(sustain×50, 15step) + R6(soft-lift+100) + R7(time-0.01) + **R8(ground contact -5.0/step, oh<0.05 gate, gripper+wrist sensor)**. **Drop detection(v7.1)**: oh<0.04 & ee_3d>0.15 → R3 회수(-100), milestone/R2 budget 리셋. 체크포인트: 10 iter. `--bc_checkpoint --object_usd --skill approach_and_grasp --headless` |
| `collect_resip_demos.py` | ~252 | ResiP expert rollout → HDF5 저장. Frozen DP + trained Residual → combined policy, 성공 에피소드만 저장. `--bc_checkpoint --resip_checkpoint --num_episodes 1000` |
| `eval_resip.py` | ~300 | ResiP 평가 스크립트. per_dim_action_scale 매칭 필수. **★ --object_usd 필수** (없으면 물체 미스폰 → 허수 LIFT 판정). `--bc_checkpoint --resip_checkpoint --object_usd --skill approach_and_grasp --num_envs 1024 --headless` |
| `vlm_orchestrator.py` | ~128 | VLM 오케스트레이터: RelativePlacementOrchestrator, classify_user_request(VLM `/classify` 호출). **Task 분류는 VLM 텍스트 호출** — 키워드 매칭 제거, Qwen2.5-VL이 relative_placement + grasp_object/dest_object 추출 |
| `vlm_prompts.py` | ~72 | VLM System Prompt 상수: RELATIVE_PLACEMENT_SYSTEM_PROMPT, NAVIGATE_SYSTEM_PROMPT |
| `vlm_inference_server.py` | — | VLM 추론 서버 (A100, Qwen2.5-VL-7B, HTTP API). **2개 엔드포인트**: `/infer`(이미지+텍스트 → instruction+phase, 실시간 오케스트레이션), `/classify`(텍스트 only → mode+target_object, task 시작 시 1회) |
| `vla_inference_server.py` | — | VLA 추론 서버 (A100, π0-FAST, HTTP API) |

### 2-2. 기존 파일 (수정됨)

| 파일 | 줄수 | 수정 내용 |
|------|------|-----------|
| `models.py` | 121 | CriticNet 추가 (AAC용) |
| `train_lekiwi.py` | ~665 | `--skill` 분기 (navigate/approach_and_grasp/carry_and_place), BC warm-start, **AAC 통합**, Navigate PPO config |
| `train_bc.py` | ~446 | `--expected_obs_dim` 필수, v6 action 이름 수정, **GMM loss 기본** (`--loss gmm --n_components 5`), `bc_nav.pt`(RL warm-start용) + `bc_nav_gmm.pt`(full GMM) 이중 저장 |
| `collect_demos.py` | ~1331 | `--skill` 분기 (navigate 추가), gripper continuous, **Skill1/2/3EnvWithCam 추가**, Navigate action override, **inference-time masking** |
| `convert_hdf5_to_lerobot_v3.py` | ~528 | `convert_to_vla_units()` passthrough, `infer_robot_state_from_obs()` dim==20 추가 |
| `record_teleop.py` | ~1265 | `--skill` 분기 (+ **combined** 추가), v6 action format, `arm_limit_write_to_sim=False`, wrist_roll 디버그 출력, **combined 듀얼 HDF5 + phase-aware 레코딩**, **`--resume` 이어서 녹화**, **`--dest_object_usd`** (빨간 컵 background 스폰), contact sensor 기반 hold 판정, **objZ 실시간 표시** (combined + legacy 모드) |
| `isaac_teleop.py` | 261 | `SIGNS[4]=1.814` (wrist_roll 기어비 실측), wrist_roll leader_raw 디버그 출력 |
...
### 3-2. lekiwi_skill2_env.py (~1900줄)

Skill-2 ApproachAndGrasp 환경. FSM 없이 단일 목표(approach→grasp→lift) 수행.
**Dest object 스폰**: `dest_object_usd` config로 목적지 물체(빨간 컵)를 background로 스폰 (`_create_extra_scene_objects()`). Skill-3는 이를 상속하여 place 목적지로 사용.
**Audit Fix**: curriculum 초기화 config 읽기 (H2), `_critic_obs` 저장 (C2).
**Reward 개선**: (1) wider gripper shaping — 0.35~0.70m 범위에서도 `0.3 × close_progress` 보상 추가 (기존 <0.35m에서만 작동), (2) height penalty — `-0.1 × clamp(|z_offset| - 0.10, min=0)` 추가하여 위에서 접근하는 비효율적 행동 방지.
**PhysX 물리 수정 (2026-02-26)**:
- `object_scale: 0.7` — 모든 물체 70% 스케일
- mass에서 `scale³` 제거 — `obj_mass = catalog mass` (기존 `mass × scale³`에서 변경, 0.7³=0.343× 질량 축소로 관통 시 폭발적 가속 유발)
- `max_depenetration_velocity: 0.3` (기존 1.0, multi-object/single-object 모두)
- `CollisionPropertiesCfg(contact_offset=0.005, rest_offset=0.002)` 추가 — 충돌 감지 정밀도 개선
- single-object 모드: USD에서 bbox 자동 계산 (`_single_object_bbox`, pxr UsdGeom 사용)
- **bbox center offset**: `_object_bbox_center_local` — USD origin → geometric center 보정. `_compute_metrics()`에서 `quat_apply(obj_quat, offset)`로 `object_pos_w`를 bbox center로 보정 (USD `root_pos_w`는 actor frame origin으로, 물체 바닥일 수 있음)
- env drop detection 제거 — 정상 파지 오판 문제 (train_resip.py reward-level에서 별도 구현)
- gripper PD overshoot 제거 — 표준 `clamp(target, arm_lo, arm_hi)`로 변경
- **FixedJoint 완전 제거** — 순수 마찰 grasp만 사용. `_grasp_fixed_joints`, `_attach_grasp_fixed_joint_for_envs()`, `_disable_grasp_fixed_joint_for_envs()`, `_teleport_attach_for_envs()`, break_force/torque DR 전부 삭제
- **EE position**: `Wrist_Roll_08c_v1` body + local offset `(0.04029, 0.0309, -0.06419)` via `quat_apply`. `_fixed_jaw_body_idx` + `_ee_local_offset` 멤버
- **Grasp 판정**: `gripper_closed(< 0.65) & has_contact(> 0.7N) & between_jaws(EE < grasp_ee_max_dist=0.07)`. `grasp_require_contact=True` config로 제어
- **Contact sensor**: `force_matrix_w` 사용 (filter_prim_paths_expr로 물체만 필터). `net_forces_w` fallback 제거 (바닥 접촉 포함 문제)
- **Ground contact sensor (R8용)**: `CuboidCfg(size=(100,100,0.02), kinematic_enabled=True)` ground + **gripper+wrist 2개 센서** (`filter_prim_paths_expr=["/World/ground"]`). `_ground_contact_force_per_env()` 메서드 + extras에 `ground_contact_force_raw` 노출
...
**주요 섹션:**
- `Skill2EnvCfg` (62-165): 환경 설정, 순수 마찰 grasp config, curriculum
- `__init__` (167-319): 인덱스, 버퍼, EE body/offset, catalog 초기화
- `_setup_scene` (325-580): 로봇/물체/센서 등록, CuboidCfg ground, gripper+wrist ground contact sensor
- Calibration/Dynamics (461-782): JSON 기반 dynamics 적용
- DR (942-1117): wheel/arm stiffness-damping (break_force DR 제거)
- `_contact_force_per_env` / `_ground_contact_force_per_env`: 물체/지면 접촉력
- `_compute_metrics`: object 상대 위치/거리/속도 + bbox center offset 보정
- `_update_grasp_state`: EE offset + contact 기반 grasp (FixedJoint 제거)
- `_apply_action` (1320-1361): v6 순서 `[arm5, grip1, base3]`
- `_get_observations` (1367-1415): 30D actor + 37D critic
- `_get_rewards` (1421-1476): approach progress + **wider gripper shaping**(0.35~0.70m) + **height penalty** + grasp bonus + lift bonus
...
주요 코드 발췌 — Action 순서 (v6, body frame → IK 변환):
...
주요 코드 발췌 — Grasp break 감지 (제거됨):

> **Drop detection 제거**: Skill-2의 `_update_grasp_state()`에서 거리 기반 drop 감지 코드가 제거되었다.
> 이유: gripper body 중심~물체 중심 자연 오프셋(~0.18m)이 존재하여 정상 파지 상태에서도
> drop으로 오판하는 문제가 있었다. `just_dropped` 필드와 `_gripper_body_idx`는 Skill-3에서
> intentional place 판정에 사용하므로 필드 자체는 유지된다.
...
class Skill2EnvCfg(DirectRLEnvCfg):
    ...
    object_dist_min: float = 0.8
    object_dist_max: float = 1.2    # LeKiwi 소형 로봇, arm reach ~0.4m 고려
    spawn_object_in_front: bool = True       # 로봇 yaw를 물체 방향으로 설정
    spawn_heading_noise_std: float = 0.35    # ~20deg Gaussian noise
    spawn_heading_max_rad: float = 0.76      # clamp (= base cam FOV half ~43.5deg)
    curriculum_success_threshold: float = 0.7
    curriculum_dist_increment: float = 0.25
    curriculum_current_max_dist: float = 1.2  # 처음부터 전체 범위 사용

    grasp_joint_break_force: float = 30.0    # mass*g*10
    grasp_joint_break_torque: float = 30.0
    grasp_drop_detect_dist: float = 0.30
...
### 3-3. lekiwi_skill3_env.py (~622줄)

Skill-3 CarryAndPlace 환경. Skill2Env를 상속하여 "잡은 물체를 목적지 물체(destination object)까지 운반 후 옆에 놓기" 수행.
...
**Combined Mode 추가**: `_combined_mode` 플래그, `_compute_skill2_actor_obs()` 메서드, `_reset_idx` combined 분기.
**Config 수정 (2026-02-26)**: `place_gripper_threshold: 0.6` (기존 0.3, grip=0.34에서 의도치 않은 place 트리거 방지), `dest_spawn_dist: 4.0~5.0m` (기존 2.0~3.0m), `near_dest: place_radius×3.0` (기존 `dest_spawn_dist_max` — 항상 true 버그 수정).

```python
"""
LeKiwi Skill-3 — CarryAndPlace Isaac Lab DirectRLEnv.

3-Skill 파이프라인의 세 번째 스킬: 잡은 물체를 들고 목적지 물체(destination object)까지 운반 후 옆에 놓기(Place).
...
Observation (Actor 29D):
  ...
  [21:24] dest_object_rel pos body (3)
  [24:25] grip force (1)
...
class Skill3EnvCfg(Skill2EnvCfg):
    ...
    # Place (목적지 물체 — destination object)
    place_radius: float = 0.20      # place 판정 XY 반경
    place_height_tolerance: float = 0.10  # place 판정 Z 허용 오차
    dest_spawn_dist_min: float = 4.0  # env origin으로부터 최소 거리
    dest_spawn_dist_max: float = 5.0  # env origin으로부터 최대 거리
    dest_object_min_sep: float = 0.5  # grasp 물체-dest 물체 최소 분리 거리
...
class Skill3Env(Skill2Env):
    ...
    def _update_grasp_state(self, metrics: Dict[str, torch.Tensor]):
        ...
        목적지 물체(destination object) 근처에서 gripper를 열면 FixedJoint를 해제하여
        물체를 의도적으로 내려놓는다. 이때 just_dropped=False를 유지하여 place_success
        조건이 충족될 수 있도록 한다 (accidental drop과 구분).
        """
        self.intentional_placed[:] = False

        # 부모(Skill2) grasp 로직 실행 (can_grasp, lift — drop detection 제거됨)
        super()._update_grasp_state(metrics)

        # Intentional place: 물체를 잡은 상태 + gripper open + dest object 근처
        if self.object_grasped.any():
            ...
            if dest_dist is None:
                ...
                dest_dist = torch.norm(dest_pos_b[:, :2], dim=-1)
            near_dest = dest_dist < (self.cfg.place_radius * 3.0)  # 0.6m
    ...
    def _get_observations(self) -> dict:
        ...
        # dest_object_rel: body-frame 상대 벡터 3D
        dest_delta_w = self.dest_object_pos_w - self.robot.data.root_pos_w
        ...
        actor_obs = torch.cat([
            ...
            dest_object_rel,            # [21:24] dest_object_rel 3D
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Place 성공: 목적지 물체(빨간 컵) 옆에 의도적으로 놓음 (drop이 아닌 경우)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        metrics = self._compute_metrics()
        self._update_grasp_state(metrics)  # intentional place 포함
        ...
        # Use _check_place_success (dest-object-based 3D check) instead.
        place_success = self._check_place_success()
        self.task_success = place_success
    ...
    # ═══════════════════════════════════════════════════════════════════
    #  Place Success Check (dest-object-based 3D)
    # ═══════════════════════════════════════════════════════════════════

    def _check_place_success(self) -> torch.Tensor:
        """목적지 물체(빨간 컵) 옆에 물체가 놓였는지 3D 판정.

        조건: (1) XY 거리 < place_radius, (2) Z가 dest 물체 높이 허용 범위 이내,
              (3) gripper open, (4) accidental drop이 아님.
        """
        obj_xy = self.object_pos_w[:, :2]
        dest_xy = self.dest_object_pos_w[:, :2]
        xy_dist = torch.norm(obj_xy - dest_xy, dim=-1)
        in_xy = xy_dist < self.cfg.place_radius

        obj_z = self.object_pos_w[:, 2]
        dest_z = self.dest_object_pos_w[:, 2]
        in_z = torch.abs(obj_z - dest_z) < self.cfg.place_height_tolerance

        gripper_pos = self.robot.data.joint_pos[:, self.gripper_idx]
        gripper_open = gripper_pos > float(self.cfg.place_gripper_threshold)

        return in_xy & in_z & gripper_open & (~self.object_grasped) & (~self.just_dropped)

    def _spawn_dest_object(self, env_ids: torch.Tensor):
        """env origin 근처에 목적지 물체 위치를 랜덤 스폰."""
        num = len(env_ids)
        ...
        self.dest_object_pos_w[env_ids, 0] = env_origins[:, 0] + dists * torch.cos(angles)
        self.dest_object_pos_w[env_ids, 1] = env_origins[:, 1] + dists * torch.sin(angles)
        self.dest_object_pos_w[env_ids, 2] = env_origins[:, 2]
    ...
    def _reset_from_handoff(self, env_ids: torch.Tensor, num: int):
        ...
        # dest object 랜덤 스폰 (env origin 근처 4.0~5.0m, grasp 물체와 최소 0.5m 분리)
        self._spawn_dest_object(env_ids)
        # home_pos_w는 out_of_bounds 판정용으로만 유지
...
`record_teleop.py --skill combined`에서 Skill-2→3 연속 텔레옵 레코딩을 위한 기능:

**1) `_combined_mode` 플래그 (`__init__`)**:
```python
self._combined_mode: bool = False  # record_teleop에서 True로 설정
```

**2) `_reset_idx` combined 분기**: Combined 모드일 때 Skill-3의 handoff 리셋 대신 Skill-2 리셋을 사용 (물체를 거리에 배치, 미파지 상태에서 시작):
```python
def _reset_idx(self, env_ids):
    if self._combined_mode:
        Skill2Env._reset_idx(self, env_ids)  # 물체 전방 배치 + DR
        self.prev_dest_dist[env_ids] = 10.0
        self.intentional_placed[env_ids] = False
        ...
        return
    # ... 기존 Skill-3 리셋 (handoff/fallback) ...
```

**3) `_compute_skill2_actor_obs()` 메서드**: Phase 1에서 Skill-2의 30D obs layout을 Skill-3 env의 메트릭으로 재현:
```python
def _compute_skill2_actor_obs(self) -> torch.Tensor:
    """30D: arm(5)+grip(1)+base_body_vel(3)+lin_vel(3)+ang_vel(3)+arm_vel(6)+object_pos_b(3)+contact(2)+bbox(3)+cat(1)"""
    metrics = self._compute_metrics()
    # ... Skill-2의 object_pos_b, contact_lr 포함, dest_object_rel/grip_force 미포함
    return torch.cat([...], dim=-1)  # 30D (Skill-2 Actor obs와 동일)
```

**왜 Skill-3 env를 사용하는가**: Skill-3의 `_get_dones()`는 lift에서 truncate하지 않음(place에서만). 따라서 Phase 1에서 물체를 잡아 들어도 env 리셋이 발생하지 않는다. Skill-3가 Skill-2를 상속하므로 모든 Skill-2 메트릭/센서 접근이 가능하다.

---

### 3-4. generate_handoff_buffer.py (178줄)
...
### 3-5. train_lekiwi.py (~665줄)

PPO 학습 스크립트. `--skill` 인자로 환경 분기, BC warm-start 지원.
**Audit Fix**: AAC 전면 통합 — `aac_wrapper` + `AAC_PPO` + `AACSequentialTrainer` (C2).
**BC Auxiliary Loss 추가**: Skill-2/3에서 frozen BC 정책의 출력을 PPO actor loss에 보조 손실로 추가.

> 전체 코드: `train_lekiwi.py` 파일 직접 참조 (~665줄).

**주요 기능:**
- `--skill` 분기: `navigate` → Skill1Env, `approach_and_grasp` → Skill2Env, `carry_and_place` → Skill3Env, `legacy` → LeKiwiNavEnv
...
- **BC Auxiliary Loss** (Skill-2/3): `--lambda_bc_init 0.5` (초기 BC loss weight), `--bc_anneal_ratio 0.6` (학습 60% 동안 λ annealing). frozen BC 정책을 로드하여 `aac_ppo.py`의 `_update()`에서 `policy_loss += λ_bc × MSE(actor_mean, bc_target)` 추가. TensorBoard: `Loss / BC loss`, `Loss / Lambda BC`
...
# AAC wrapper for skill-1/2/3, legacy는 symmetric
...
### 3-6. train_bc.py (~447줄)

Behavioral Cloning 학습. **GMM(Gaussian Mixture Model) loss가 기본** (`--loss gmm --n_components 5`).
MSE의 mean regression 문제를 해결하여 극단값 보존. `BCPolicy` 구조가 `PolicyNet`과 동일하여 weight transfer 가능.

> 전체 코드: `train_bc.py` 파일 직접 참조 (~447줄).

**주요 기능:**
- `BCPolicy`: `net(obs→256→128→64) + mean_layer(64→9)` — PolicyNet과 동일 (MSE용)
- `BCPolicyGMM`: backbone 동일 + GMM heads (`pi_layer`, `mu_layer`, `log_std_layer`) — K개 Gaussian 컴포넌트로 다봉 분포 학습
- `gmm_nll_loss()`: GMM Negative Log-Likelihood 손실 함수
- `extract_bc_state_dict()`: GMM에서 dominant component의 mu → `mean_layer`로 변환, RL warm-start 호환 state dict 추출
- `--loss gmm` 기본 (권장), `--n_components 5` 기본
- `--expected_obs_dim` 필수: 20(Skill-1), 30(Skill-2), 29(Skill-3) 검증
- `--filter_active`: `teleop_active=True`인 프레임만 사용 (idle 프레임 제거, 텔레옵 데이터 권장)
- 기본 정규화 OFF (PPO RunningStandardScaler와 정합)
- stale norm 파일 자동 삭제
- **이중 체크포인트 저장**: `bc_nav.pt` (RL warm-start용, BCPolicy 호환) + `bc_nav_gmm.pt` (GMM 전체, eval_bc용)

```python
class BCPolicy(nn.Module):
    """MSE용 — PolicyNet.net + mean_layer과 동일 구조."""
...
class BCPolicyGMM(nn.Module):
    """GMM용 — K개 Gaussian 컴포넌트로 다봉 분포 학습."""
    def __init__(self, obs_dim: int = 33, act_dim: int = 9, n_components: int = 5):
        super().__init__()
        self.n_components, self.act_dim = n_components, act_dim
        self.net = nn.Sequential(  # backbone: BCPolicy와 동일
            nn.Linear(obs_dim, 256), nn.ELU(),
            nn.Linear(256, 128), nn.ELU(),
            nn.Linear(128, 64), nn.ELU(),
        )
        self.pi_layer = nn.Linear(64, n_components)              # mixture weights
        self.mu_layer = nn.Linear(64, n_components * act_dim)    # component means
        self.log_std_layer = nn.Linear(64, n_components * act_dim)  # component stds
    ...
    def extract_bc_state_dict(self, obs_sample=None):
        """RL warm-start용 BCPolicy 호환 state dict 추출.
        backbone 복사 + dominant component의 mu → mean_layer 변환."""
        sd = {f"net.{k}": v.clone() for k, v in self.net.state_dict().items()}
        # dominant component 찾기
        if obs_sample is not None and len(obs_sample) > 0:
            with torch.no_grad():
                pi_logits, _, _ = self.forward(obs_sample)
                dominant_k = int(pi_logits.argmax(dim=-1).mode().values.item())
        else:
            dominant_k = 0
        K, D = self.n_components, self.act_dim
        w = self.mu_layer.weight.data.view(K, D, -1)
        b = self.mu_layer.bias.data.view(K, D)
        sd["mean_layer.weight"] = w[dominant_k].clone()
        sd["mean_layer.bias"] = b[dominant_k].clone()
        return sd


def gmm_nll_loss(pi_logits, mu, log_std, target):
    ...
    log_normal = log_normal.sum(dim=-1)  # (B, K)
...
### 3-7. collect_demos.py (~1331줄)

RL Expert 데모 수집. `--skill` 분기 지원, camera + SpawnManager 통합.
**Audit Fix**: `Skill2EnvWithCam`, `Skill3EnvWithCam` 카메라 서브클래스 추가 (H1).

> 전체 코드: `collect_demos.py` 파일 직접 참조 (~1331줄).

**주요 기능:**
- `--skill` 분기: navigate / approach_and_grasp / carry_and_place / legacy
- `extract_robot_state_9d()`: `[arm_pos(6), body_vel(3)]` 9D
  - body velocity: `root_lin_vel_b` / `root_ang_vel_b` 직접 읽기
- Gripper continuous (이진화 제거 — RL/BC 전 파이프라인 연속값)
- **Inference-time masking**: Navigate skill에서 cross-axis 잔차 제거 (fwd/bwd→vy,wz=0 등)
- **Skill1EnvWithCam / Skill2EnvWithCam / Skill3EnvWithCam**: base_rgb (1280x720) + wrist_rgb (640x480) 카메라 서브클래스
- SpawnManager: 물체 스폰/리스폰, lighting DR
- HDF5 구조: obs, actions, robot_state, subtask_ids, images

```python
# 카메라 서브클래스 (Audit H1 수정 + Navigate 추가):
class Skill1EnvWithCam(Skill1Env):
    """Skill1Env + base_rgb/wrist RGB 카메라 (VLA 데이터 수집용)."""
    # Skill2EnvWithCam과 동일 구조
...
### 3-8. collect_navigate_data.py (600줄, fallback)

Skill-1 Navigate 데이터 수집 (스크립트 정책 방식, fallback). RL Expert rollout은 `collect_demos.py --skill navigate`를 사용. Script policy (P-control) + noise injection.

> 전체 코드: `collect_navigate_data.py` 파일 직접 참조 (600줄).
...
### 3-9. convert_hdf5_to_lerobot_v3.py (528줄)

HDF5 → LeRobot v3 dataset 변환기.

> 전체 코드: `convert_hdf5_to_lerobot_v3.py` 파일 직접 참조 (528줄).
...
- `infer_robot_state_from_obs()`: robot_state 없을 때 obs에서 추출 (20D/30D/29D/37D/33D/24D)
...
### 3-10. record_teleop.py (~1265줄)

텔레옵 데모 녹화. ROS2/TCP 듀얼 입력, `--skill` 분기 지원.

> 전체 코드: `record_teleop.py` 파일 직접 참조 (~1265줄).

**주요 기능:**
- `--skill`: approach_and_grasp (Skill-2, 30D obs) / carry_and_place (Skill-3, 29D obs) / **combined** (3-phase: Skill-2→Transit→Skill-3, 듀얼 HDF5) / legacy (v8 FSM)
- `--resume`: 기존 HDF5에 이어서 녹화. 단일 skill 모드에서는 `--output` 필수. Combined 모드에서는 `--output` 불필요 — `demos/` 내 최신 `combined_skill2_*.hdf5`를 자동 탐색하고, skill3 파일은 "skill2"→"skill3" 변환으로 유도. HDF5를 append 모드로 열고, 기존 에피소드 수를 카운트하여 이어서 번호를 매김. `--num_demos`는 총 목표 수 (기존 포함)
- `--grasp_hold_steps`: combined 모드 전용, Phase 1→2 전환 파지 유지 스텝 수 (기본 450 = 7.5s @ 60Hz)
- `--dest_object_usd`: 목적지 물체(빨간 컵) USD 경로. 설정 시 env에 background rigid body로 스폰
- `--home_dist_thresh`: combined 모드 전용, Phase 2→3 전환 거리 (기본 0.7m). 실제 목적지는 `dest_object_pos_w` (있으면) 또는 `home_pos_w`
- `--home_fov_thresh`: combined 모드 전용, Phase 2→3 전환 FOV 각도 (기본 0.76rad)
...
`--skill combined` 시 Skill3Env를 `_combined_mode=True`로 생성하고, `grasp_gripper_threshold=0.65`으로 설정.
...
**듀얼 HDF5 출력:**
- `demos/combined_skill2_TIMESTAMP.hdf5` — Phase 1 에피소드 (obs_dim=30)
- `demos/combined_skill3_TIMESTAMP.hdf5` — Phase 3 에피소드 (obs_dim=29)

**3-Phase 스텝 루프:**
```
Phase 1 (Skill-2 기록):
  매 스텝 env._compute_skill2_actor_obs()로 30D obs 기록 (PRE-step: env.step() 이전에 캡처)
  contact sensor + gripper_closed 직접 체크 (object_grasped 미사용) → grasp_hold_steps 도달 시:
    → Skill-2 에피소드 저장 → Phase 2 전환
    → episode_length_buf = 0 (Transit 타이머 리셋)

Phase 2 (Transit, 미기록):
  매 스텝 episode_length_buf = 0 (timeout 방지)
  사람이 키보드로 목적지 물체(빨간 컵) 근처까지 이동
  dest_dist < 0.7m AND |heading_to_dest| < 0.76rad 시:
    → Phase 3 전환
    → episode_length_buf = 0 (Skill-3 타이머 리셋)
  drop/fall 시: 리셋 → Phase 1

Phase 3 (Skill-3 기록):
  매 스텝 obs["policy"] (29D) 기록
  done 시:
    place_success → Skill-3 에피소드 저장
    drop/timeout → Skill-3 폐기
  → 리셋 → Phase 1로

Phase 1에서 done (timeout/fall):
  → 데이터 폐기 → 리셋 → Phase 1 재시도
```

**Phase 2→3 전환 조건 (목적지=dest object, `_dest_attr` 패턴 사용):**
```python
_dest_attr = "dest_object_pos_w" if hasattr(env, "dest_object_pos_w") else "home_pos_w"
_dest_pos = getattr(env, _dest_attr)
dest_delta_w = _dest_pos - root_pos_w
dest_rel_b = quat_apply_inverse(root_quat, dest_delta_w)
dest_dist = torch.norm(dest_rel_b[:2]).item()
heading_to_dest = math.atan2(dest_rel_b[0].item(), dest_rel_b[1].item())  # +y forward
transition = dest_dist < 0.7 and abs(heading_to_dest) < 0.76
```
...
**aac_ppo.py (358줄):**
- `AAC_PPO(PPO)`: critic에 별도의 privileged observation 전달 + **BC auxiliary loss 지원**
- `__init__()`: `critic_states` memory 텐서 생성, `_critic_state_preprocessor` 초기화, `bc_policy`/`lambda_bc_init`/`bc_anneal_ratio` 저장
- `record_transition()`: `_current_critic_states`로 value 계산, memory에 `critic_states` 저장
- `_update()`: full PPO._update() override — `sampled_critic_states`로 critic 학습, **BC auxiliary loss 추가**: `policy_loss = surrogate_loss + λ_bc × MSE(actor_mean, bc_target)`. λ_bc는 `lambda_bc_init`에서 시작하여 `bc_anneal_ratio` 비율의 학습 구간 동안 선형 감소 → 0
- TensorBoard 로그: `Loss / BC loss`, `Loss / Lambda BC`

```python
class AAC_PPO(PPO):
    def __init__(self, ..., critic_observation_space=None,
                 bc_policy=None, lambda_bc_init=0.0, bc_anneal_ratio=0.6):
        super().__init__(...)
        ...
        # BC auxiliary loss
        self._bc_policy = bc_policy          # frozen BC policy (None이면 BC loss 미적용)
        self._lambda_bc_init = lambda_bc_init
        self._bc_anneal_ratio = bc_anneal_ratio
    ...
    def _update(self, timestep, timesteps):
        ...
        # BC auxiliary loss (λ annealing)
        if self._bc_policy is not None and self._lambda_bc_init > 0:
            progress = timestep / timesteps
            anneal_end = self._bc_anneal_ratio
            lambda_bc = self._lambda_bc_init * max(0.0, 1.0 - progress / anneal_end)
            with torch.no_grad():
                bc_target, _, _ = self._bc_policy.act({"states": sampled_states}, ...)
            bc_loss = F.mse_loss(actor_mean, bc_target)
            policy_loss = surrogate_loss + lambda_bc * bc_loss
            # TensorBoard: "Loss / BC loss", "Loss / Lambda BC"
```
...
@dataclass
class EarlyStopCfg:
    metric: str = ""          # extras["log"] 키 — rolling avg >= threshold
    threshold: float = 0.93
    metric2: str = ""         # rolling avg <= threshold2
    threshold2: float = 0.05
    extra_ge: list = None     # 추가 >= 조건 [(metric_name, threshold), ...]
    window: int = 500
...
class AACSequentialTrainer(SequentialTrainer):
    def __init__(self, *args, early_stop: EarlyStopCfg | None = None, **kwargs):
        ...
        self._es_buffer = deque(maxlen=w)       # metric1
        self._es_buffer2 = deque(maxlen=w)      # metric2
        self._es_extra_buffers = {name: deque(maxlen=w) for name, _ in extra_ge}
    ...
    def single_agent_train(self):
        ...
        for timestep in range(self.initial_timestep, self.timesteps):
            ...
            # Early stopping: 모든 조건 만족 시 학습 중단
            # cond1: metric1 avg >= threshold (예: rew_track_ang >= 1.35)
            # cond2: metric2 avg <= threshold2 (예: ang_vel_error <= 0.02)
            # cond_extra: extra_ge의 모든 (metric, threshold) avg >= threshold
```

Navigate는 `train_lekiwi.py`에서 `--skill navigate` 시 자동으로 4개 조건 활성화:
`EarlyStopCfg(metric="rew_track_ang", threshold=1.35, metric2="ang_vel_error", threshold2=0.02, extra_ge=[("avg_speed", 0.25), ("direction_compliance", 0.55)])`.
수동 설정: `--early_stop_metric <key> --early_stop_threshold <val> --early_stop_metric2 <key2> --early_stop_threshold2 <val2> --early_stop_window 500`.
...
### Combined 텔레옵 (Skill-2 → Transit → Skill-3 연속 레코딩)

```bash
# 3-phase: Skill-2(접근+파지) → Transit(미기록 이동) → Skill-3(목적지 접근+place)
# Phase 1→2: 물체를 잡고 7.5초(450스텝) 유지 (contact sensor + gripper_closed 기준)
# Phase 2→3: 목적지 물체(빨간 컵)가 0.7m 이내 + FOV 내
python record_teleop.py --skill combined --num_demos 10 \
    --grasp_hold_steps 450 \
    --home_dist_thresh 0.7 --home_fov_thresh 0.76 \
    --object_usd /path/to/object/model_clean.usd \
    --dest_object_usd /path/to/ACE_Coffee_Mug_.../model_clean.usd \
    --gripper_contact_prim_path "/World/envs/env_.*/Robot/LeKiwi/Moving_Jaw_08d_v1" \
    --arm_limit_json calibration/arm_limits_measured.json

# 출력: demos/combined_skill2_TIMESTAMP.hdf5 (30D obs)
#        demos/combined_skill3_TIMESTAMP.hdf5 (29D obs)
```
...
python record_teleop.py --num_demos 20 --skill approach_and_grasp \
    --output demos/skill2_session01.hdf5 \
    --multi_object_json object_catalog.json \
    --gripper_contact_prim_path "/World/envs/env_.*/Robot/LeKiwi/Moving_Jaw_08d_v1"

# 중단 후 이어서 녹화 (--resume)
python record_teleop.py --num_demos 20 --skill approach_and_grasp \
    --output demos/skill2_session01.hdf5 --resume \
    --multi_object_json object_catalog.json \
    --gripper_contact_prim_path "/World/envs/env_.*/Robot/LeKiwi/Moving_Jaw_08d_v1"
```
...
| ID | 심각도 | 제목 | 수정 파일 | 상태 |
|----|--------|------|----------|------|
| CRITICAL-1 | Critical | `grasp_gripper_threshold=-0.3` 도달 불가 | `lekiwi_skill2_env.py`, `train_lekiwi.py`, `collect_demos.py`, `record_teleop.py`, `lekiwi_nav_env.py` | 완료 — Skill2Cfg=0.65, CLI default=0.7 (물체 파지 시 0.4~0.8 rad 대응) |
...
| AUDIT-V2-2 | Critical | _reset_from_handoff: env_origin 복원 + dest_object_pos_w 스폰 + home_pos_w(out_of_bounds용) 수정 | `lekiwi_skill3_env.py` | 완료 |
...
| AUDIT-V2-7 | Critical | Action body→IK 좌표계 변환 | `lekiwi_skill1_env.py`, `lekiwi_skill2_env.py` | 완료 |
| AUDIT-V2-8 | High | Cross-velocity penalty 추가 (perpendicular vel 억제) | `lekiwi_skill1_env.py` | 완료 |
| AUDIT-V2-9 | Critical | wz 부호 규약 수정: direction_cmd turn 부호 반전 + `ik_wz=-body_wz` → `ik_wz=body_wz` | `lekiwi_skill1_env.py`, `lekiwi_skill2_env.py` | 완료 (v4) |
| AUDIT-V2-10 | Critical | Reward 재설계: compliance+cross_vel 혼합 → Isaac Lab style lin/ang exp kernel 분리. 정규화 비대칭 근본 해소 | `lekiwi_skill1_env.py` | 완료 (v6) |
| AUDIT-V2-11 | High | Turn cmd magnitude ±1.0 → ±0.33 (target wz 3.0→1.0 rad/s). 물리적 달성 불가능한 target 수정 | `lekiwi_skill1_env.py` | 완료 (v6b) |
| AUDIT-V2-12 | High | ang tracking std 0.5→0.25, weight 0.75→1.5. wz drift penalty 4배 강화. Early stop: rew_track_ang>=1.35, ang_vel_error<=0.02 | `lekiwi_skill1_env.py`, `train_lekiwi.py` | 완료 (v6c) |
| AUDIT-V2-13 | High | Wz drift penalty 추가: linear cmd(fwd/bwd/strafe)에서 `rew_wz_drift_penalty=-2.0 * wz²`. 정책이 뒷바퀴만 돌려 strafing하는 exploit 방지 시도 | `lekiwi_skill1_env.py` | 롤백 (v6d→v6c, wz_drift 없이 v6c가 성능 우수) |
| AUDIT-V2-14 | Medium | 순수 velocity tracker로 전환: collision(-2.0)/proximity(-0.5) reward 제거, ang_std 0.25→0.20 (lin_std=0.25 유지). 장애물 회피는 VLA가 담당, RL은 방향 충실 실행만 | `lekiwi_skill1_env.py` | 완료 (v6e) |
| ~~AUDIT-V2-14b~~ | — | ~~v6f: turn_lin_drift penalty -3.0 추가~~ | — | 롤백 (v6f 실패: 회전 개선됐으나 직진 성능 하락, 비대칭 gradient 간섭) |
| AUDIT-V2-17 | High | lin_std 0.25→0.15 축소 (v7): exp kernel tighter tracking. turn 시 0.05 m/s drift → 11% reward loss (v6e의 4%에서 개선). turn_lin_drift penalty 없이 대칭적으로 모든 방향에서 억제 | `lekiwi_skill1_env.py` | 완료 (v7, v6g로 대체) |
| AUDIT-V2-18 | High | Navigate v8(action masking) 실패 → **v6c 복귀**: action masking은 1/3 sample efficiency로 compliance 0.34 정체. v6c(simple tracking + collision/proximity, masking 없음)로 학습, **inference-time masking**으로 eval/deployment에서만 cross-axis 잔차 제거. v6c vs v6f 비교 후 최종 선택 (더 이상 실험 안 함) | `lekiwi_skill1_env.py`, `eval_navigate.py`, `collect_demos.py` | 확정 (v6c 학습중, v6f 비교 예정) |
| AUDIT-V2-19 | High | BC Auxiliary Loss: frozen BC 정책의 MSE를 PPO actor loss에 추가. `--lambda_bc_init 0.5 --bc_anneal_ratio 0.6`. λ 선형 감소. TensorBoard: `Loss / BC loss`, `Loss / Lambda BC` | `train_lekiwi.py`, `aac_ppo.py` | 완료 |
| AUDIT-V2-20 | Medium | Skill-2 reward 개선: wider gripper shaping(0.35~0.70m도 0.3×close_progress), height penalty(-0.1×clamp(\|z_offset\|-0.10, min=0)) | `lekiwi_skill2_env.py` | 완료 |
| AUDIT-V2-15 | High | Combined Phase 1 off-by-one 수정: `_compute_skill2_actor_obs()`를 `env.step()` 이전에 캡처하여 MDP 정합 (s_t, a_t) 보장. Phase 3/single-skill은 기존에도 정확 | `record_teleop.py` | 완료 |
| AUDIT-V2-16 | Medium | `train_bc.py`에 `--filter_active` 추가: `teleop_active=True` 프레임만 사용, idle 프레임 제거. HDF5에 이미 저장된 teleop_active 활용 | `train_bc.py` | 완료 |

---

## 6. Navigate 체크포인트 백업 현황 (2026-02-25)

| 파일 | 버전 | 상태 | 비고 |
|------|------|------|------|
| `best_agent_v6c.pt` | v6c | 학습중 (216K iter) | **현재 코드**: simple tracking + collision/proximity, masking 없음 |
| `best_agent_v6e.pt` | v6e | 완주 | collision/proximity 제거, 순수 velocity tracker |
| `best_agent_v6f.pt` | v6f | 완주 | v6c + turn_drift penalty (v6c와 비교 대상) |
| `best_agent_v6g2.pt` | v6g2 | 17K steps 중간 | ortho=-1.0, linear penalty (폐기) |
| `best_agent_v6g3.pt` | v6g3 | 완주 | ortho=-15.0, quadratic penalty (폐기) |
| `best_agent_v8.pt` | v8 | 195K iter | action masking, compliance 0.34 정체 (폐기) |

**최종 결정**: v6c vs v6f 비교 후 선택, 더 이상 실험하지 않음. Inference-time masking으로 cross-axis drift 제거 (학습 시 masking 없음, eval/deployment만).

---

## 7. Phase 4.5 비교 실험 계획

**목적**: 3-Skill Hierarchical 파이프라인의 가치를 다양한 베이스라인과 정량 비교

**Task**: "약병 찾아서 빨간 컵 옆에 놓아" — Navigate(약병)→Grasp(약병)→Navigate(빨간 컵)→Place(빨간 컵 옆)

| 실험 | 구조 | 사람 데모 | RL | Mimic 증강 | 비고 |
|------|------|-----------|-----|-----------|------|
| A1 | E2E baseline (VLM 없음) | 전체 task 텔레옵 50개 | 없음 | 없음 | 텔레옵 → π0-FAST fine-tuning |
| B1 | VLM+VLA 텔레옵 only | skill별 20~40개 | 없음 | 없음 | VLM 오케스트레이션 + skill별 VLA |
| B2 | VLM+VLA 텔레옵+Mimic 증강 | skill별 20~40개 | 없음 | 있음 | B1 + Mimic augmentation |
| C1 | VLM+VLA+RL (우리 파이프라인) | skill별 20~40개 | Skill-1 Navigate | 없음 | 3-Skill hierarchical |
| C2 | VLM+VLA+RL+Mimic | skill별 20~40개 | Skill-1 Navigate | 있음 | C1 + Mimic augmentation |

비교 항목: 성공률, 사람 데모 효율, 새 물체/환경 일반화, 구현 복잡도. Phase 4.5 sim 평가에서 동일 조건(물체, 환경, 시작점) 각 30회+ 수행.
...
- [🔄] Skill-1 v6c 학습중 (A100, 216K iter) — v6f와 비교 후 최종 선택. Inference-time masking 적용
...
- [ ] vlm_inference_server.py 구현 (Qwen2.5-VL-7B HTTP API)
- [ ] vla_inference_server.py 구현 (π0-FAST HTTP API)
- [ ] **비교 실험**: A1/B1/B2/C1/C2 5개 조건 비교 (Phase 4.5)
- [ ] Sim Full-System 평가 실행 및 Go 기준 달성
- [ ] 실기 배포 테스트
