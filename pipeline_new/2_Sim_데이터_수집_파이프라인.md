...
**Skill-1(Navigate)은 독립 환경** (`lekiwi_skill1_env.py`): Direction-conditioned RL — VLM 방향 명령(forward/backward/left/right/turn_left/turn_right)을 obs로 받아, 해당 방향으로 충실히 이동하는 순수 velocity tracker를 학습한다. 텐서 기반 장애물 + 8-ray pseudo-lidar로 구성 (장애물 스폰은 유지하되, 장애물 회피는 VLA가 담당). `train_lekiwi.py --skill navigate`로 학습. **Action은 body frame** (`vx=right, vy=forward, wz>0=CCW per Isaac Sim right-hand rule`)으로 해석하며, `_apply_action()` 내부에서 body→IK 변환 수행 (`ik_wz=body_wz`, 동일 CCW+ 규약). **보상 (Isaac Lab style, v6g)**: v6f base(lin_std=0.25, turn_lin_drift=-3.0) + orthogonal drift penalty(`ortho_drift_weight=-1.0`). lin_tracking(1.5, lin_std=0.25) + ang_tracking(1.5, ang_std=0.20) + ortho_drift(-1.0) + smoothness(-0.005) + time(-0.01). collision/proximity reward 제거. orthogonal drift penalty가 목표 방향에 직교하는 속도 성분을 억제 (`ortho_speed` TensorBoard 로그). **direction_cmd turn magnitude**: ±0.33 (target wz = 1.0 rad/s). Early stop: `rew_track_ang >= 1.35 AND ang_vel_error <= 0.02`.

**Navigate v6g 비교 실험 현황 (2026-02-24)**:
- **핵심 이슈**: 전후진(forward/backward) 시 뒷바퀴 drift 발생. 좌우이동(strafe)과 회전은 정상 동작.
- **v6g2** (`ortho_drift_weight=-1.0`, linear penalty): 현재 코드 상태 (`lekiwi_skill1_env.py`). 서버에서 v6g2_full 처음부터 완주 학습 진행중.
- **v6g3** (`ortho_drift_weight=-15.0`, quadratic penalty): 완주됨 → `backup/best_agent_v6g3.pt`
- 비교 완료 후 최종 버전을 선정하여 Skill-1 데이터 수집(Phase 2)에 사용.
- **체크포인트 백업**: `best_agent_v6e.pt`, `best_agent_v6g2.pt`(17K steps 중간), `best_agent_v6g3.pt`(완주)
...
Phase 1: RL Expert 학습 (텔레옵: 3090 Desktop, BC/RL: A100 서버)
  Skill-1 (Navigate):
    [🔄] RL v6g 진행 중 (A100, 8192 envs, v6f base + ortho_drift_weight=-1.0, lin_std=0.25 ang_std=0.20 weight=1.5, turn cmd ±0.33, early stop: rew_track_ang>=1.35 AND ang_vel_error<=0.02)
