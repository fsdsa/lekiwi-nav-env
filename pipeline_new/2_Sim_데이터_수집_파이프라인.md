...
**Skill-1(Navigate)은 독립 환경** (`lekiwi_skill1_env.py`): Direction-conditioned RL — VLM 방향 명령(forward/backward/left/right/turn_left/turn_right)을 obs로 받아, 해당 방향으로 충실히 이동하는 순수 velocity tracker를 학습한다. 텐서 기반 장애물 + 8-ray pseudo-lidar로 구성 (장애물 스폰은 유지하되, 장애물 회피는 VLA가 담당). `train_lekiwi.py --skill navigate`로 학습. **Action은 body frame** (`vx=right, vy=forward, wz>0=CCW per Isaac Sim right-hand rule`)으로 해석하며, `_apply_action()` 내부에서 body→IK 변환 수행 (`ik_wz=body_wz`, 동일 CCW+ 규약). **보상 (v8: action masking + simple tracking)**: action masking으로 명령별 불필요 dim을 0으로 강제 (fwd/bwd→vx,wz=0, strafe→vy,wz=0, turn→vx,vy=0). lin_tracking(1.5, std=0.25) + ang_tracking(1.5, std=0.25) + smoothness(-0.005, masked actions) + time(-0.01). collision/proximity reward 제거. **direction_cmd turn magnitude**: ±0.33 (target wz = 1.0 rad/s). Sample efficiency 1/3 (명령당 gradient). Early stop: `rew_track_ang >= 1.35 AND ang_vel_error <= 0.02 AND avg_speed >= 0.25 AND direction_compliance >= 0.55`.

**Navigate v8 학습 현황 (2026-02-25)**:
- **v8 (action masking)**: reward 경쟁 문제를 구조적으로 해결. A100 서버에서 216K iterations 학습 진행중.
- 이전 시도 (v6f~v7, v6g 모두 폐기): reward-based drift 억제가 tracking reward에 압도되어 실패.
- **체크포인트 백업**: `best_agent_v6e.pt`, `best_agent_v6g2.pt`, `best_agent_v6g3.pt`
...
Phase 1: RL Expert 학습 (텔레옵: 3090 Desktop, BC/RL: A100 서버)
  Skill-1 (Navigate):
    [🔄] RL v8 진행 중 (A100, 8192 envs, action masking + simple tracking, lin/ang std=0.25 weight=1.5, 216K iter, early stop: rew_track_ang>=1.35 AND ang_vel_error<=0.02 AND avg_speed>=0.25 AND direction_compliance>=0.55)
