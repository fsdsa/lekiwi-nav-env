...
- `vllm`: vLLM 0.17.0 + Qwen3-VL-8B-Instruct (VLM 추론, OpenAI-compatible API)
...
| `vllm` | vLLM 0.17.0 + Qwen3-VL-8B (VLM 추론, OpenAI API, gpu_util=0.75) | Phase 4.5 + Phase 5 | ✅ 검증 완료 |
...
**모델 캐시:**
- Qwen3-VL-8B-Instruct: `~/.cache/huggingface/hub/models--Qwen--Qwen3-VL-8B-Instruct` (다운로드 완료)
...
```
vLLM (Qwen3-VL-8B bf16):  --gpu-memory-utilization 0.75 → ~29.8GB 선점
Pi0-FAST 2.9B (VLA):       ~8.1GB 사용 (lerobot 0.5.0)
합계:                      ~37.9GB / 40GB
```
> `--gpu-memory-utilization 0.75`: Qwen3-VL-8B는 0.50으로는 KV cache OOM 발생. 0.75 필수.
...
| VLM Navigate | vLLM + Qwen3-VL-8B (2-4Hz) | D455 base_cam 640×400 → 방향 command |
