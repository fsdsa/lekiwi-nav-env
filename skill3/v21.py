...
def main_combined():
    ...
    V15_PLACED_DECAY_STEPS = 150.0  # PLACED ×50 시간감쇠 (NEAR_PLACED와 동일 패턴) — 무한 farm 차단
    ...
    v15_post_rel_ct = torch.zeros(N, dtype=torch.long, device=dev)  # v20: released 후 경과 step (PLACED 감쇠)
    ...
    while gs < args.total_timesteps:
        ...
        for step in range(S):
            ...
            if is_s3.any() and s3_v15:
                ...
                # ═══════════════════════════════════════
                # R4. UPRIGHT ×8 (세워놓기 보상)
                # 팔 내리는 동안 물체 upright ≈ 1.0 유지
                # src_h ∈ [0.04, 0.10]: lowering 구간에서만
                # ═══════════════════════════════════════
                upright_active = active_h & near_dest & (src_h < 0.10) & (src_h >= 0.04) & (~v15_ms_released)
                rew[upright_active] += (8.0 * src_upright)[upright_active]

                # ═══════════════════════════════════════
                # R5. GRIP OPEN ×8 (그리퍼 열기 보상)
                # 물체가 바닥(src_h < 0.055)에 있고 서있을때(upright > 0.95)
                # grip 열기 → Phase C 시작
                # ═══════════════════════════════════════
                ...
                rew[safe_open] += (8.0 * grip_open_q)[safe_open]
                ...
                # NEAR_PLACED (시간 감쇠): 바닥에서 물체 잡고 유지 (released 전)
                #
                # v19f(안정): PLACED ×200 → V(floor) 높음 → lowered 280/iter
                # v19i/j(진동): NEAR_PLACED ×30~50 고정 → V(floor) 낮음 → lowered 진동
                #
                # 해결: ×100에서 시작, 150 step에 걸쳐 ×0으로 감쇠
                # - 초반: ×100 → V(floor) 높음 → 강한 lowering attractor
                # - 시간 경과: 보상 감소 → "여기 계속 있으면 손해"
                # - R5 grip(×8) + M3(+150)이 자연스러운 탈출구
                #
                # Math (500 remaining steps):
                #   Hold forever: ∫₀¹⁵⁰ 100(1-t/150)dt = 7,500 then 0 = 7,500
                #   Release @75:  ~5,600 + M3(150) + PLACED 50×425 + RETRACT + M4 + Suc ≈ 34,000
                #   → release 4.5배 더 높음
                near_placed = active_h & near_dest & (src_h < 0.06) & (src_upright > 0.85) & v15_ms_lowered & (~v15_ms_released)
                near_placed_q = torch.exp(-0.5 * (placement_err / 0.10) ** 2) * src_upright
                near_placed_decay = torch.clamp(1.0 - v15_lower_sustain.float() / 150.0, 0.0, 1.0)
                rew[near_placed] += (100.0 * near_placed_decay * near_placed_q)[near_placed]

                # PLACED ×50: released 후 (물체 상태만 체크, grip 무관)
                # retract 중 BC grip 닫혀도 물체 서있으면 보상 유지
                # v20: 시간감쇠 추가 (NEAR_PLACED 패턴). 무감쇠 ×50/step은 v19v 붕괴의 주 farm
                #   (release 후 안 닫고 앉아 수금 — 정상 retract+close ~100 step엔 full 보상 유지)
                v15_post_rel_ct[s3m & v15_ms_released] += 1
                placed_decay = torch.clamp(
                    1.0 - v15_post_rel_ct.float() / V15_PLACED_DECAY_STEPS, 0.0, 1.0)
                placed_post_release = placed_object & v15_ms_released
                rew[placed_post_release] += (50.0 * placed_q * placed_decay)[placed_post_release]
                ...
                if safe_open.any():
                    iter_s3_reward_grip += float((8.0 * grip_open_q)[safe_open].sum().item())
