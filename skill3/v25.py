...
parser.add_argument("--logstd_anneal_to", type=float, default=0.0,
                    help="v23: 학습 후반 logstd 천장을 이 값까지 선형 점감 (0=비활성). "
                         "mean-policy 갭(결정론 eval에서 하강/접근 정체) 봉합용")
parser.add_argument("--logstd_anneal_start", type=float, default=0.3,
                    help="v23: 전체 iter 중 anneal 시작 비율")
...
from skill3_bc_obs import build_s3_bc_obs, ee_world_pos
...
def main_combined():
    ...
    # 4-phase sustained counters (training data와 동일한 전환 기준)
    s3_arm_rise_ct = torch.zeros(N, dtype=torch.long, device=dev)    # A→B: arm1 sustained
    s3_near_dest_ct = torch.zeros(N, dtype=torch.long, device=dev)   # v25: A→B fallback (주차 데드락 해소)
    s3_grip_open_ct = torch.zeros(N, dtype=torch.long, device=dev)   # B→C: grip sustained
    s3_retract_ct = torch.zeros(N, dtype=torch.long, device=dev)     # C→D: retract sustained
    ...
    S3_MAX_STEPS = 2000       # v25: 3000→2000. 데모 T≈1100; 막다른길(부정확 배치 등) 낭비 절감→시도 횟수↑
    ...
    # ── v20: EXPLOIT-PROOF retract→close tail ──
    # v19u 문제 ① 단일 6DOF 동시 수렴 → 진동  ② grip 게이트 없는 success → 안 닫음
    # v19v 붕괴 (iter~28, gclose→0, ret_mean 4.7→330, D-time 0→16%):
    #   close 게이트(성공 거부 레버) + grip 권한 1.5(BC close 차단력)
    #   + per-step 절대 보상 farm(PLACED ×50/step 무감쇠, rest hold +20/step)
    #   → "앉아서 farming"이 success(+500)보다 큼 → close 소멸
    # v20 원칙: release 이후 per-step 절대 보상 금지 — potential Δ(텔레스코핑) / 시간감쇠 / one-time만
    V15_ARM_VELPEN         = 0.15   # arm joint-velocity 패널티 (복귀 진동 직접 억제)
    V15_CARRY_ARM_VELPEN   = 0.05   # 운반/내림 중 arm velocity 패널티 (grasp slip→drop 감소)
    V15_GRIP_CLOSE_W       = 40.0   # grip 닫기 potential-Δ 가중치
    V15_GRIP_CLOSED_MAX    = 0.05   # grip_pos < 이 값 = "닫힘"(milestone)
    V15_GRIP_CLOSE_SUSTAIN = 8      # grip-closed milestone sustain step
    V15_GRIP_CLOSE_SCALE   = 0.30   # grip residual 권한: BC close를 못 막는 수준 (1.5는 차단 가능했음)
    ...
    V15_REST_TOL = 0.70        # v19v: 1.00→0.70 (success가 grip-closed도 요구; arm이 실제로 rest 근처여야)
    ...
    v15_ms_grip_closed = torch.zeros(N, dtype=torch.bool, device=dev)  # v19v: rest 후 grip 닫힘 완료
    v15_ms_success = torch.zeros(N, dtype=torch.bool, device=dev)
    # Post-success hold: success 후 rest pose 학습 시간 (Skill-2 구조)
    S3_SUCCESS_HOLD_STEPS = 150
    s3_success_hold_counter = torch.zeros(N, dtype=torch.long, device=dev)
    ...
    v15_grip_close_sustain = torch.zeros(N, dtype=torch.long, device=dev)  # v19v
    v15_prev_grip_dist = torch.zeros(N, device=dev)  # v20: |grip−REST_GRIP| prev (close potential-Δ)
    v15_prev_grip_open_q = torch.zeros(N, device=dev)  # v24: R5 Δ용 prev
    v15_prev_upright = torch.ones(N, device=dev)       # v24: R4 tilt-loss Δ용 prev
    ...
    while gs < args.total_timesteps:
        ...
        # Phase별 residual scale — 4-phase (A/B/C/D)
        # A: carry+approach → arm 절대 안 건드림, base만 RL
        # B: arm lowering   → arm RL로 lowering 보정, grip은 닫힘 유지
        # C: grip open      → grip RL로 열기, arm 미세 조정
        # D: retract         → arm RL로 rest pose 복귀
        s3_scale_a = torch.zeros(S3_AD, device=dev)
        s3_scale_a[0:5] = 0.0     # arm: carry 자세 절대 보호
        s3_scale_a[5]   = 0.0     # grip: carry 보호
        s3_scale_a[6:9] = 0.40    # base: dest 접근

        s3_scale_b = torch.zeros(S3_AD, device=dev)
        s3_scale_b[0:5] = 0.05    # arm: BC lowering 거의 안 건드림 (v19d: 0.20→0.05)
        s3_scale_b[5]   = 0.0     # grip: holding 유지
        s3_scale_b[6:9] = 0.20    # v25: 0.10→0.20. fallback 진입 시 하강 중 base 정렬 보정력 필요

        s3_scale_c = torch.zeros(S3_AD, device=dev)
        s3_scale_c[0:5] = 0.05    # arm: 거의 안 건드림
        s3_scale_c[5]   = 0.50    # grip: RL이 열기 학습
        s3_scale_c[6:9] = 0.05    # base: 미세 조정

        s3_scale_d = torch.zeros(S3_AD, device=dev)
        s3_scale_d[0:5] = 0.10    # arm: BC retract 보호 (Phase B arm과 같은 논리)
        s3_scale_d[5]   = 0.10    # grip: BC grip closing 보호
        s3_scale_d[6:9] = 0.05    # base: 미세 조정
        ...
        # v23: logstd anneal — 후반 탐색 노이즈 점감 → mean-policy 갭 봉합
        #   (v21 iter10 결정론 eval: timeout 52%, 다수가 하강 모션 미시작(arm1Δ≈0) —
        #    노이즈가 행동을 깨우는 구조라 mean은 고착. 후반에 mean을 직접 최적화)
        _lsa_cap = None
        if args.logstd_anneal_to != 0.0:
            _p = max(0.0, min(1.0, (gi / max(NI, 1) - args.logstd_anneal_start)
                              / max(1.0 - args.logstd_anneal_start, 1e-6)))
            _lsa_cap = args.init_logstd + (args.logstd_anneal_to - args.init_logstd) * _p
            with torch.no_grad():
                rpol.actor_logstd.data.clamp_(max=_lsa_cap)

        iter_header = (
            f"\nIter {gi}/{NI} | {'EVAL' if ev else 'TRAIN'} | step={gs} | "
            f"S2:{(phase==0).sum().item()} S3:{(phase==1).sum().item()}"
        )
        if _lsa_cap is not None:
            iter_header += f" | logstd_cap={_lsa_cap:.2f}"
        if s3_stage == "place_hold":
            iter_header += (
        ...
        v15_iter_released = 0
        v15_iter_rested = 0
        v15_iter_grip_closed = 0
        v15_iter_success = 0
        # 4-phase monitoring counters
        iter_s3_pha_steps = 0
        iter_s3_phb_steps = 0
        iter_s3_phc_steps = 0
        iter_s3_phd_steps = 0
        ...
        iter_s3_bc_trans = 0
        iter_s3_cd_trans = 0
        iter_s3_pha_vy_sum = 0.0
        iter_s3_pha_vy_count = 0
        iter_s3_reward_approach = 0.0
        iter_s3_reward_height = 0.0
        iter_s3_reward_place = 0.0
        iter_s3_reward_grip = 0.0
        iter_s3_reward_retract = 0.0
        for step in range(S):
            if not ev: gs += N
            ...
            with torch.no_grad():
                ...
                _grip_cur = env.env.robot.data.joint_pos[:, env.env.arm_idx[5]]
                _init_arm1 = s3_init_pose6[:, 1]
                _cf_cur = env.env._contact_force_per_env()
                _holding_cur = (_cf_cur > float(env.env.cfg.grasp_contact_threshold)) & (_grip_cur < float(env.env.cfg.grasp_gripper_threshold))
                _is_s3 = (phase == 1)

                # ── 4-phase obs flags (training data와 동일) ──
                # A→B: arm1 > init+0.10 sustained 5 steps AND base_dst < 0.40
                # (데모에서 arm lowering은 반드시 dest 근처 도착 후 시작됨.
                #  BC arm drift로 인한 조기 전환 방지 — 이것은 임시 방편이 아닌
                #  데모의 실제 행동 패턴을 반영한 정당한 조건)
                _arm_rising = _is_s3 & s3_phase_a_latch & (_arm1_cur > _init_arm1 + 0.10)
                s3_arm_rise_ct[_arm_rising] += 1
                s3_arm_rise_ct[~_arm_rising] = 0
                # v25: A→B fallback — v24 eval 정체 모드 수정.
                #   BC는 phase_a=0을 봐야 하강 시작 (flag 조건부 행동), 래치는 arm 상승을
                #   기다림 = 데드락 (v21 B→C와 동일 구조). 결정론 정책은 dest 0.36m에
                #   '주차'된 채 영구 대기 (eval timeout 56%의 주성분).
                #   → 근처(bdxy<0.45) 300 step 지속 시 강제 flag-flip. BC가 하강 시도,
                #     base는 B에서 계속 정렬(scale 0.20), 품질은 M-게이트가 강제.
                s3_near_dest_ct[_is_s3 & s3_phase_a_latch & (bdxy < 0.45)] += 1
                s3_near_dest_ct[~(_is_s3 & s3_phase_a_latch & (bdxy < 0.45))] = 0
                entered_b = _is_s3 & s3_phase_a_latch & (
                    ((s3_arm_rise_ct >= 5) & (bdxy < 0.40))
                    | (s3_near_dest_ct >= 300)
                )
                if entered_b.any():
                ...
                # B→C (v21, 데모 flag 온셋 역산으로 보정):
                #   구판 grip>0.60×5는 데드락 — BC는 flag를 봐야 여는 걸 배웠는데(데모에서
                #   flag52/53은 "병이 바닥 최종 위치 + 아직 파지" 시점에 켜짐: h=0.033,
                #   sdxy p50=0.137, grip=0.23 closed), 학습은 BC가 열어야 flag를 줬음.
                #   → lowered-complete 조건으로 전환: holding & 바닥 & 목적지 근처, ×5
                _src_h_cur = env.env.object_pos_w[:, 2] - env.env.scene.env_origins[:, 2]
                _sdxy_cur = torch.norm(
                    env.env.object_pos_w[:, :2] - env.env.dest_object_pos_w[:, :2], dim=-1)
                _in_b = _is_s3 & (~s3_phase_a_latch) & (~s3_place_open_latch)
                _lowered_done = _in_b & _holding_cur & (_src_h_cur < 0.058) & (_sdxy_cur < 0.25)  # v25: 0.05→0.058 (eval env15: 0.051 호버 — 1mm 차로 영구 대기)
                s3_grip_open_ct[_lowered_done] += 1
                s3_grip_open_ct[~_lowered_done & _in_b] = 0
                _new_c = _in_b & (s3_grip_open_ct >= 5)
                s3_place_open_latch[_new_c] = True
                s3_release_phase_latch[_new_c] = True

                # C→D (v21): 데모 flag54 온셋 = "팔 rest + grip 재폐쇄 완료" (grip=-0.12,
                #   arm=REST, frac 0.66). 구판 grip>=0.95 wide-open은 정반대 타이밍이었고
                #   데모 92%는 grip을 1.0 이상 열지도 않음 (release grip p50=0.46).
                _arm5_cur = env.env.robot.data.joint_pos[:, env.env.arm_idx[:5]]
                _rest_cur = torch.norm(_arm5_cur - V15_REST_POSE.unsqueeze(0), dim=-1)
                _in_c = _is_s3 & s3_place_open_latch & (~s3_retract_started_latch)
                _retract_ready = _in_c & (_rest_cur < 0.70) & (_grip_cur < 0.05)
                s3_retract_ct[_retract_ready] += 1
                s3_retract_ct[~_retract_ready & _in_c] = 0
                _new_d = _in_c & (s3_retract_ct >= 3)
                s3_retract_started_latch[_new_d] = True
            ...
            # ── 4-phase residual scale selection (flag 기반) ──
            _is_a = s3_phase_a_latch
            _is_d = s3_retract_started_latch
            _is_c = s3_place_open_latch & (~_is_d)
            _is_b = (~_is_a) & (~s3_place_open_latch)

            s3_scale = torch.zeros(N, S3_AD, device=dev)
            s3_scale[_is_a] = s3_scale_a.unsqueeze(0)
            s3_scale[_is_b] = s3_scale_b.unsqueeze(0)
            s3_scale[_is_c] = s3_scale_c.unsqueeze(0)
            s3_scale[_is_d] = s3_scale_d.unsqueeze(0)

            # v19v: arm이 rest 도달하면 Phase-D 그리퍼 권한↑ (0.10으론 open→closed swing 불가)
            _is_d_close = _is_d & v15_ms_rested
            if _is_d_close.any():
                s3_scale[_is_d_close, 5] = V15_GRIP_CLOSE_SCALE

            # Phase B 조건부 grip: 바닥 도달 시에만 grip RL 허용 (v19f)
            # 높은 곳에서 grip noise → drop 방지 (v19e 문제)
            ...
            # Phase B: grip scale=0 → RL 영향 없음, BC 자연 출력 (바닥 근처에서 grip open)
            # Phase C/D: RL 제어 (scale 0.50/0.30)
            # Phase A drop은 S2 expert의 grasp 품질 문제이지 grip clamp으로 해결할 문제 아님
            ...
            if transition.any():
                ...
                s3_arm_rise_ct[t_ids] = 0
                s3_near_dest_ct[t_ids] = 0
                s3_grip_open_ct[t_ids] = 0
                s3_retract_ct[t_ids] = 0
                s3_success_hold_counter[t_ids] = 0
                ...
                if s3_v15:
                    ...
                    v15_ms_grip_closed[t_ids] = False
                    v15_ms_success[t_ids] = False
                    v15_lower_sustain[t_ids] = 0
                    v15_rest_sustain[t_ids] = 0
                    v15_grip_close_sustain[t_ids] = 0
                    v15_release_sustain[t_ids] = 0
                    v15_prev_grip_dist[t_ids] = (
                        env.env.robot.data.joint_pos[t_ids, env.env.gripper_idx] - V15_REST_GRIP
                    ).abs()
                    v15_prev_grip_open_q[t_ids] = torch.clamp(
                        (env.env.robot.data.joint_pos[t_ids, env.env.gripper_idx] - 0.25) / 0.25, 0.0, 1.0)
                    v15_prev_upright[t_ids] = 1.0
            ...
            if is_s3.any() and s3_v15:
                # ═════════════════════════════════════════════════════════
                # PHASE-AWARE REWARD STRUCTURE (BC eval 분석 기반 재설계)
                #
                # BC eval (lr=1e-4 ep500) 병목 분석:
                #   ① 접근 79%    → approach ×8 + Phase A base scale 0.40
                #   ② 팔 내림 51% → height ×40 + placement ×20
                #   ③ 바닥 38%    → lowered milestone +100
                #   ④ 그립 열림 19% → grip open ×8 (기존 ×3에서 대폭 증가)
                #   ⑤ retract 13% → retract ×40 + rest milestone +150
                #
                # 데모 기준 좌표:
                #   V15_TARGET_SRC_B = [-0.122, 0.380] (물체 body-frame 목표)
                #   V15_TARGET_DST_B = [+0.013, 0.380] (dest body-frame 기준)
                #   placement offset = src - dst ≈ [-0.135, 0]
                #   바닥 높이: src_h ≈ 0.033
                #   upright > 0.95 at placement
                # ═════════════════════════════════════════════════════════
                s3m = is_s3
                ...
                # Placement offset (데모 기준 3-body 상대좌표)
                ...
                near_dest = dst_body_dist < 0.20

                # ── Phase masks (flag 기반) ──
                _phA = active & s3_phase_a_latch
                _phB = active & (~s3_phase_a_latch) & (~s3_place_open_latch)
                _phC = active & s3_place_open_latch & (~s3_retract_started_latch)
                _phD = active & s3_retract_started_latch

                # ── Phase monitoring ──
                iter_s3_pha_steps += int(_phA.sum().item())
                iter_s3_phb_steps += int(_phB.sum().item())
                iter_s3_phc_steps += int(_phC.sum().item())
                iter_s3_phd_steps += int(_phD.sum().item())
                if entered_b.any():
                    iter_s3_ab_trans += int(entered_b.sum().item())
                if _new_c.any():
                    iter_s3_bc_trans += int(_new_c.sum().item())
                if _new_d.any():
                    iter_s3_cd_trans += int(_new_d.sum().item())

                # ═══════════════════════════════════════
                # R1. APPROACH ×8 (접근 보상)
                # 물체를 들고 dest 방향으로 이동 → dst_body_dist 감소
                # Phase A에서 가장 중요. Phase B에서도 미세 조정용
                # ═══════════════════════════════════════
                dst_delta = torch.clamp(v15_prev_dst_body_dist - dst_body_dist, -0.05, 0.05)
                rew[active_h] += (8.0 * dst_delta)[active_h]

                # ═══════════════════════════════════════
                # R2. HEIGHT LOWERING ×40 (팔 내림 보상)
                # near_dest 도착 후, holding 상태에서 src_h 감소
                # src_h > 0.04: 바닥에 너무 가까우면 height delta 보상 끄기
                # ═══════════════════════════════════════
                lowering_active = active_h & near_dest & (src_h > 0.04)
                height_delta = torch.clamp(prev_src_h - src_h, -0.02, 0.02)
                rew[lowering_active] += (40.0 * height_delta)[lowering_active]

                # ═══════════════════════════════════════
                # R3. PLACEMENT CONVERGENCE ×20 (위치 수렴 보상)
                # 로봇-컵-약병 3-body 상대좌표가 데모 기준에 수렴
                # placement_err = ||(src-dst)_body - target_offset||
                # ═══════════════════════════════════════
                placing = active_h & near_dest & (src_h < 0.10)
                place_delta = torch.clamp(v15_prev_src_body_dist - placement_err, -0.05, 0.05)
                rew[placing] += (20.0 * place_delta)[placing]

                # ═══════════════════════════════════════
                # R4 (v24: Δ-패널티로 전환). 구판 +8×upright/step 절대 보상은
                # 하강 밴드 호버 farm 가능 → "기울어짐 손실"만 벌점 (유지=0, telescoping)
                # ═══════════════════════════════════════
                upright_active = active_h & near_dest & (src_h < 0.10) & (~v15_ms_released)
                _tilt_loss = torch.clamp(v15_prev_upright - src_upright, min=0.0)
                rew[upright_active] += (-8.0 * _tilt_loss)[upright_active]

                # ═══════════════════════════════════════
                # R5 (v24: potential-Δ로 전환) — v23 붕괴의 주범 수정.
                # 구판 +8×grip_open_q/step 절대 보상: M3가 안 뜨는 부정확 배치에선
                # ~ms_released 영구 참 → 무한 지급 (iter당 grip=16만~26만 vs approach ~670,
                # 전체 보상의 99%). "그리퍼 열림=돈" gradient가 close를 직접 억압
                # → rest+ 유지·gclose만 사멸 (iter15 86% → iter34 13%).
                # Δ형: 열기 "모션"만 보상, 총합 ≤8, 왕복 telescoping 0 → farm 불가
                # ═══════════════════════════════════════
                safe_open = active & near_dest & (src_h < 0.055) & (src_upright > 0.95) & (~v15_ms_released)
                grip_open_q = torch.clamp((grip_pos - 0.25) / 0.25, 0.0, 1.0)  # v21: 데모 release grip(p50 0.46)에 보정
                grip_open_delta = grip_open_q - v15_prev_grip_open_q
                rew[safe_open] += (8.0 * grip_open_delta)[safe_open]

                # ═══════════════════════════════════════
                # R6. PLACED / NEAR_PLACED (배치 보상)
                #
                # NEAR_PLACED ×30/step (released 전):
                #   바닥 근처 + 서있음 + holding → 바닥에서 grip open 시도 유도
                #   v19h 문제: 바닥 도달 후 지속 보상 없어서 lowering 포기
                #   ×30: R5 grip(×8)과 합쳐서 ~35/step → 충분한 유지 인센티브
                #   but PLACED ×200보다 작아서 "grip 안 열고 버티기" 최적 아님
                #   (v19g 문제: PLACED ×200이 grip open 없이도 줘서 B→C 차단)
                #
                # PLACED ×200/step (released 후):
                #   grip open 완료 → 물체 서있는 상태 유지 보상
                #   released가 GATE — grip 안 열면 접근 불가
                # ═══════════════════════════════════════
                # placed_object: 물체 상태 기반 배치 판정 (v21: placed_for_release 폐기 —
                #   grip>0.60 절대 임계가 데모 release grip 분포(p50 0.46) 밖이었음)
                #
                # placed_object: 물체 상태만 (grip 무관)
                #   → PLACED post-release 보상에서 사용
                #   → retract 중 BC가 grip 닫아도 물체가 서있으면 보상 유지
                #   → (v19p 문제: grip<0.60→placed=False→PLACED=0→retract 손해)
                placed_object = active & (placement_err < 0.15) & (src_h < 0.06) & (src_upright > 0.70) & v15_ms_lowered

                # ── v22: NEAR_PLACED / PLACED per-step 윈도우 완전 제거 ──
                # 위 구판 주석의 "release가 4.5배 이득" 산수는 RAW 단위 기준 —
                # normalize_reward(std 나눔 + clip±10)가 위계를 뒤집음:
                #   · per-step 윈도우: step마다 따로 clip → ~10/step × 100+ step = +1000 보존
                #   · one-time milestone: 한 방에 clip → success(+500)도 M1(+50)도 똑같이 +10
                # → 정규화 후 경제학 = "close(=success 리셋, 윈도우 상실)" 손해
                # → v21 iter 20+에서 rel+ 유지·gclose/suc만 사멸한 직접 원인 (v20 후반 동일)
                # 대체: NEAR_PLACED 기능은 M2 tiers(+10/30/60/150 one-time)가 이미 수행,
                #       PLACED 기능(retract 중 물체 보호)은 drop 감지(-5+리셋)가 수행

                # ═══════════════════════════════════════
                # v19v: SEQUENTIAL retract → close (arm / gripper 분리)
                #   D1  arm  → REST_POSE   (released 이후 항상)
                #   D1b arm joint-velocity damping (복귀 중 진동 직접 억제)
                #   D2  grip → REST_GRIP   (arm이 rest 도달 후에만 = 순차 게이트)
                # v19u 문제: 단일 6DOF 목표 동시 수렴 → 진동 / grip 약·무게이트 → 안 닫힘
                # ═══════════════════════════════════════
                retract_active = active & v15_ms_released
                arm_rested_now = retract_active & v15_ms_rested   # D2 gate (순차)

                # D1. ARM RETRACT — v19w: POTENTIAL(Δ)만. 절대 위치 보상(rest_q) 삭제.
                #   γ=0.999에서 per-step 절대 보상 = farmable → rest에 '앉아' close 안 하고 farming
                #   (v19v 붕괴: D-time 0%→16%, gclose→0). retract 구동 = Δ + M4(+150) + BC + vel damping.
                rest_delta = torch.clamp(v15_prev_rest_dist - rest_dist, -0.05, 0.05)
                rew[retract_active] += (20.0 * rest_delta)[retract_active]

                # D1b. ARM VELOCITY DAMPING — 복귀 중 떨림/오버슈트 직접 억제 (v19u엔 없던 항)
                _arm_vel = torch.nan_to_num(
                    env.env.robot.data.joint_vel[:, env.env.arm_idx[:5]], nan=0.0)
                _arm_vel_pen = (_arm_vel ** 2).sum(dim=-1)
                rew[retract_active] += (-V15_ARM_VELPEN * _arm_vel_pen)[retract_active]

                # D2. GRIP CLOSE — arm rest 후에만(순차), v20: potential-Δ (per-step 절대 보상 금지).
                #     텔레스코핑: 열었다 닫아도 합 0 → farm 불가. 닫기 완주 시 총 ~W×1.2 + M5.
                grip_dist = (grip_pos - V15_REST_GRIP).abs()
                grip_close_delta = v15_prev_grip_dist - grip_dist  # 물리 속도로 자연 바운드
                rew[arm_rested_now] += (V15_GRIP_CLOSE_W * grip_close_delta)[arm_rested_now]

                # FIX3 (v19w): 운반·내림 중 arm을 부드럽게 → grasp slip(drop) 감소.
                #   carry(holding & ~released) arm joint velocity 패널티 (S3엔 없던 regularization).
                #   _arm_vel_pen은 위 D1b에서 전체 env에 대해 계산됨 (재사용).
                _carry_smooth = active_h & (~v15_ms_released)
                rew[_carry_smooth] += (-V15_CARRY_ARM_VELPEN * _arm_vel_pen)[_carry_smooth]

                # ═════════════════════════════════════════
                # MILESTONES (one-time)
                # ═════════════════════════════════════════

                # M1: Base aligned — dst_body_dist < 0.12 (+50)
                ...
                # M2: Lowered — near_dest, arm near lower_pose, src_h < 0.10, sustained 5 steps (+100)
                # v21: lower_dist(팔 자세 매칭) 제거 — 데모 87%가 다른 팔 자세로 내려놓음 (13%만 매칭)
                in_lowered = active_h & near_dest & (src_h < 0.06)
                ...
                # M2 tiers: sustained lowering bonuses
                ...
                # M3: Released (v21 데이터 보정) — 데모 release grip p50=0.46, 92%가 1.0 미만
                #   → grip>1.0 절대 임계 폐기. 기능적 판정: 물체가 제자리에 서있고(placed_object)
                #     접촉 끊김(~holding) + grip이 carry(~0.23)보다 열림(>0.30), sustained 10
                _grip_open_enough = active & placed_object & (~holding) & (grip_pos > 0.30)
                ...
                # M4: Rested — rest_dist < 1.0 sustained 3 steps (+150)
                # 전체 파이프라인 학습 우선: ~60% retract만 해도 통과
                # R7+R9가 계속 full retract 유도, M4는 경험 확보용
                _rest_close = v15_ms_released & (rest_dist < V15_REST_TOL)
                v15_rest_sustain[_rest_close] += 1
                _rest_not_close = s3m & ~_rest_close & (v15_rest_sustain > 0)
                v15_rest_sustain[_rest_not_close] = torch.clamp(v15_rest_sustain[_rest_not_close] - 3, min=0)
                new_rest = active & _rest_close & (v15_rest_sustain >= 3) & (~v15_ms_rested)
                if new_rest.any():
                    rew[new_rest] += 150.0
                    v15_ms_rested[new_rest] = True
                    v15_iter_rested += int(new_rest.sum().item())

                # M5: Grip closed — v15_ms_rested 이후 grip이 닫힘 유지 (순차 close 완료, +150)
                _grip_is_closed = arm_rested_now & (grip_pos < V15_GRIP_CLOSED_MAX)
                v15_grip_close_sustain[_grip_is_closed] += 1
                _grip_reopened = s3m & ~_grip_is_closed & (v15_grip_close_sustain > 0)
                v15_grip_close_sustain[_grip_reopened] = torch.clamp(
                    v15_grip_close_sustain[_grip_reopened] - 3, min=0)
                new_gclose = (arm_rested_now
                              & (v15_grip_close_sustain >= V15_GRIP_CLOSE_SUSTAIN)
                              & (~v15_ms_grip_closed))
                if new_gclose.any():
                    rew[new_gclose] += 150.0
                    v15_ms_grip_closed[new_gclose] = True
                    v15_iter_grip_closed += int(new_gclose.sum().item())

                # ═════════════════════════════════════════
                # FINAL SUCCESS (+500)
                # ═════════════════════════════════════════
                final_success = (
                    s3m & v15_ms_rested & v15_ms_grip_closed
                ...
                if final_success.any():
                    ...
                    # 즉시 리셋하지 않음 — hold counter 시작 (Skill-2 구조)
                    s3_success_hold_counter[final_success] = S3_SUCCESS_HOLD_STEPS
                    ...
                    iter_s3_success_srcdst_sum += src_dst_xy[final_success].sum().item()
                    iter_s3_success_objz_sum += src_h[final_success].sum().item()
                    iter_s3_success_upright_sum += src_upright[final_success].sum().item()
                    iter_s3_success_arm1max_sum += s3_arm1_max_buf[final_success].sum().item()
                    print(f"    [V17] SUCCESS! {final_success.sum().item()} step={step} src_dst={src_dst_xy[final_success].mean():.3f}")

                # Post-success hold: R7+R8+R9 보상 계속 받으며 rest pose 학습
                _in_hold = s3m & v15_ms_success & (s3_success_hold_counter > 0)
                s3_success_hold_counter[_in_hold] -= 1
                # Hold 종료 → 리셋
                _hold_done = s3m & v15_ms_success & (s3_success_hold_counter <= 0)
                s3_stage_success_reset[_hold_done] = True

                # ═════════════════════════════════════════
                # PENALTIES
                # ═════════════════════════════════════════
                rew[s3m] += -0.02          # 시간 패널티 (urgency)
                rew[s3_drop] += -5.0       # drop 패널티
                rew[s3_timeout] += -2.0    # timeout 패널티
                _dm = (ter | tru).view(-1).bool()
                rew[_dm] = 0.0

                # ── Reward tracking (per-iter accumulation) ──
                if active_h.any():
                    iter_s3_reward_approach += float((8.0 * dst_delta)[active_h].sum().item())
                if lowering_active.any():
                    iter_s3_reward_height += float((40.0 * height_delta)[lowering_active].sum().item())
                if placing.any():
                    iter_s3_reward_place += float((20.0 * place_delta)[placing].sum().item())
                if safe_open.any():
                    iter_s3_reward_grip += float((8.0 * grip_open_delta)[safe_open].sum().item())
                if retract_active.any():
                    iter_s3_reward_retract += float((20.0 * rest_delta
                                                     - V15_ARM_VELPEN * _arm_vel_pen)[retract_active].sum().item())
                if arm_rested_now.any():
                    iter_s3_reward_retract += float((V15_GRIP_CLOSE_W
                                                     * grip_close_delta)[arm_rested_now].sum().item())
                ...
                v15_prev_grip_dist[s3m] = grip_dist[s3m]
                v15_prev_grip_open_q[s3m] = grip_open_q[s3m]
                v15_prev_upright[s3m] = src_upright[s3m]
            ...
            if s3_reset.any():
                ...
                s3_arm_rise_ct[reset_ids] = 0
                s3_grip_open_ct[reset_ids] = 0
                s3_retract_ct[reset_ids] = 0
                s3_success_hold_counter[reset_ids] = 0
            ...
            if reset_mask.any():
                ...
                s3_arm_rise_ct[reset_mask] = 0
                s3_grip_open_ct[reset_mask] = 0
                s3_retract_ct[reset_mask] = 0
                s3_success_hold_counter[reset_mask] = 0
        ...
        if s3_mask.any():
            ...
            if s3_stage == "place_hold":
                ...
                _v15_n_gclose = v15_ms_grip_closed[s3_mask].sum().item()
                ...
                print(
                    ...
                    f"rel={_v15_n_released} rest={_v15_n_rested} gclose={_v15_n_gclose} OK={_v15_n_success} | "
                    f"place_total={s3_place_total} drop_total={s3_drop_total}"
                )
                _hold_n = (v15_ms_success & (s3_success_hold_counter > 0) & (phase == 1)).sum().item()
                _phd_rest_dist = rest_dist[(phase == 1) & s3_retract_started_latch]
                _phd_rd_str = f" PhD_rd={_phd_rest_dist.mean():.2f}" if _phd_rest_dist.numel() > 0 else ""
                print(
                    f"  V15 IterΔ: align+={v15_iter_aligned} low+={v15_iter_lowered} "
                    f"rel+={v15_iter_released} rest+={v15_iter_rested} gclose+={v15_iter_grip_closed} "
                    f"success+={v15_iter_success} "
                    f"hold={_hold_n}{_phd_rd_str}"
                )
                # 4-phase monitoring
                _ph_total = max(iter_s3_pha_steps + iter_s3_phb_steps + iter_s3_phc_steps + iter_s3_phd_steps, 1)
                print(
                    f"  Phase dist: A={iter_s3_pha_steps}({100*iter_s3_pha_steps/_ph_total:.0f}%) "
                    f"B={iter_s3_phb_steps}({100*iter_s3_phb_steps/_ph_total:.0f}%) "
                    f"C={iter_s3_phc_steps}({100*iter_s3_phc_steps/_ph_total:.0f}%) "
                    f"D={iter_s3_phd_steps}({100*iter_s3_phd_steps/_ph_total:.0f}%) | "
                    f"trans: A→B={iter_s3_ab_trans} B→C={iter_s3_bc_trans} C→D={iter_s3_cd_trans}"
                )
                print(
                    f"  Reward: approach={iter_s3_reward_approach:.1f} "
                    f"height={iter_s3_reward_height:.1f} "
                    f"place={iter_s3_reward_place:.1f} "
                    f"grip={iter_s3_reward_grip:.1f} "
                    f"retract={iter_s3_reward_retract:.1f}"
                )
            else:
