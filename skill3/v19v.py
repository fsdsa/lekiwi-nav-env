...
def main_combined():
    ...
    # ── v19v: SEQUENTIAL retract→close tail (arm / gripper 보상 분리) ──
    # v19u 문제 ① arm+grip 단일 6DOF 목표 동시 수렴 → 복귀 중 진동
    #          ② rest 성공범위가 넓고 grip 게이트 없음 → 안 닫아도 성공
    V15_ARM_VELPEN         = 0.15   # arm joint-velocity 패널티 (복귀 진동 직접 억제)
    V15_ARM_REST_HOLD_W    = 20.0   # arm rest 도달 후 유지 보상 (grip 닫는 동안 팔 고정)
    V15_GRIP_CLOSE_W       = 40.0   # grip 닫기 보상 (v19u R8=10 → 분리·강화)
    V15_GRIP_CLOSE_SIGMA   = 0.15   # grip 닫힘 정밀 Gaussian width
    V15_GRIP_CLOSED_MAX    = 0.05   # grip_pos < 이 값 = "닫힘"(milestone)
    V15_GRIP_CLOSE_SUSTAIN = 8      # grip-closed milestone sustain step
    V15_GRIP_CLOSE_SCALE   = 1.5    # arm rest 후 Phase-D 그리퍼 residual 권한 (v19u 0.10 → 닫기 가능)
    ...
    while gs < args.total_timesteps:
        ...
        for step in range(S):
            ...
            if is_s3.any() and s3_v15:
                ...
                rew[placed_post_release] += (50.0 * placed_q)[placed_post_release]
                ...
                # D1. ARM REST — arm 5관절만 (gripper와 분리). potential delta + Gaussian.
                #     rest 도달 후엔 arm 보상을 hold 수준으로 낮춰 무게중심을 D2(close)로 이동.
                rest_q = torch.exp(-0.5 * (rest_dist / 0.50) ** 2)
                rest_delta = torch.clamp(v15_prev_rest_dist - rest_dist, -0.05, 0.05)
                _arm_w = torch.where(
                    v15_ms_rested,
                    torch.full_like(rest_q, V15_ARM_REST_HOLD_W),
                    torch.full_like(rest_q, 40.0),
                )
                rew[retract_active] += (_arm_w * rest_q)[retract_active]
                ...
                # D2. GRIP CLOSE — arm이 rest 도달 후에만 (순차). 0→1 progress + 정밀 Gaussian.
                #     tight Gaussian만 쓰면 open(>1.0)에서 gradient≈0 → far-field progress 항 필요.
                grip_prog = ((1.0 - grip_pos) / (1.0 - V15_REST_GRIP)).clamp(0.0, 1.0)
                grip_rest_err = (grip_pos - V15_REST_GRIP).abs()
                grip_rest_q = torch.exp(-0.5 * (grip_rest_err / V15_GRIP_CLOSE_SIGMA) ** 2)
                rew[arm_rested_now] += (V15_GRIP_CLOSE_W * 0.5 * grip_prog)[arm_rested_now]
                rew[arm_rested_now] += (V15_GRIP_CLOSE_W * 0.5 * grip_rest_q)[arm_rested_now]
                ...
                if retract_active.any():
                    iter_s3_reward_retract += float((_arm_w * rest_q + 20.0 * rest_delta
                                                     - V15_ARM_VELPEN * _arm_vel_pen)[retract_active].sum().item())
                if arm_rested_now.any():
                    iter_s3_reward_retract += float((V15_GRIP_CLOSE_W * 0.5
                                                     * (grip_prog + grip_rest_q))[arm_rested_now].sum().item())
