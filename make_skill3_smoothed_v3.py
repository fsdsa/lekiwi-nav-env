#!/usr/bin/env python3
"""Build a stronger Phase-B-only demo from smoothed_v2.

Goals:
- keep only episodes with sufficiently deep lowering
- keep the gripper clearly open for longer before retract
- smooth the open->close transition during retract
"""
...
def _ensure_plateau(
    ...
    # Hold the gripper clearly open from open_start to retract_start.
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("demos_skill3/smoothed_v3.hdf5"))
    parser.add_argument("--min_arm1_peak", type=float, default=2.8)
    parser.add_argument("--open_thresh", type=float, default=0.55)
    parser.add_argument("--min_open_steps", type=int, default=160)
    ...
    with h5py.File(args.input, "r") as f_in, h5py.File(args.output, "w") as f_out:
        ...
        f_out.attrs["smoothed_v3"] = True
        f_out.attrs["min_arm1_peak"] = args.min_arm1_peak
        f_out.attrs["min_open_steps"] = args.min_open_steps
        f_out.attrs["blend_steps"] = args.blend_steps
        ...
        for ep in sorted(f_in.keys(), key=_ep_sort_key):
            ...
            retract_start = _find_retract_start(arm1, open_start)

            jump_before.append(float(np.min(np.diff(actions[:, 5]))))
            open_ratio_before.append(float(np.mean(actions[:, 5] > 0.5)))
            ...
            open_ratio_after.append(float(np.mean(act2[:, 5] > 0.5)))

            dst = f_out.create_group(f"episode_{out_idx}")
            _copy_attrs(src, dst)
            dst.attrs["source_episode"] = ep
            dst.attrs["arm1_peak"] = peak
            dst.attrs["open_start"] = open_start
            dst.attrs["retract_start_orig"] = retract_start
            dst.attrs["retract_start_new"] = retract_start2
            ...
            for key in src.keys():
                if key in {"obs", "actions", "robot_state"}:
                    continue
    ...
    if inserted_steps:
        print(
            "inserted_steps mean/min/max:",
            f"{np.mean(inserted_steps):.1f} / {np.min(inserted_steps)} / {np.max(inserted_steps)}",
        )
        print(
            "open_ratio (>0.5 act) before -> after:",
            f"{np.mean(open_ratio_before):.4f} -> {np.mean(open_ratio_after):.4f}",
        )
        print(
            "min one-step grip-action delta before -> after:",
            f"{np.mean(jump_before):.4f} -> {np.mean(jump_after):.4f}",
        )
